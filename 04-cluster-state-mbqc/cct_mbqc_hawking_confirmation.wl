...
(* ===========================================================================
   cct_mbqc_hawking_confirmation.wl -- POST-BUILD CONFIRMATION STAGE.

   Confronts the agnostic first-principles Hawking build
     * cct_mbqc_hawking_evaporation.wl   (Page curve, Renyi-2, Hayden-Preskill)
     * cct_mbqc_hawking_certification.wl (CHSH / contextual-fraction stack)
   ...
   Run:  wolframscript -file cct_mbqc_hawking_confirmation.wl
   =========================================================================== *)

If[!ValueQ[CCTConfDir], CCTConfDir = DirectoryName[$InputFileName]];
...
Block[{CCTHawkingLoadOnly = True, CCTMBQCPatternsLoadOnly = True,
  ...
  Get[FileNameJoin[{CCTConfDir, "cct_mbqc_hawking_evaporation.wl"}]]];
Block[{CCTHawkingCertLoadOnly = True, CCTMBQCPatternsLoadOnly = True},
  Get[FileNameJoin[{CCTConfDir, "cct_mbqc_hawking_certification.wl"}]]];
...
Print["================================================================"];
Print[" cct_mbqc_hawking_confirmation.wl -- build vs published targets"];
Print["================================================================"];
...
(* ===========================================================================
   ...
   Our CCTPageRenyi2Closed is EXACTLY this formula in bits; the 2-design
   Clifford ensemble (CCTPageRenyi2Ensemble) reproduces <purity> exactly, so
...
Do[
  Module[{r2, peakClosedBits, peakClosedNats, peakSampledBits, peakSampledNats,
          maxdiffBits, tgt},
   SeedRandom[100 + n];
   r2 = CCTPageRenyi2Ensemble[n, t1Shots[n], "Seed" -> (100 + n)];
...
Do[Module[{pc, peakBits, closedPeakBits, closedPeakNats},
   pc = CCTPageCurveScrambledState[n, "Seed" -> n, "Sweeps" -> 14];
   peakBits = pc["peak"];
   closedPeakBits = N[CCTPageRenyi2Closed[n, n/2]];   (* exact ensemble Renyi-2 *)
...
Module[{sc, epr, det, tele, idctrl, sweep},
  ...
  idctrl = CCTHPDecodeEPR[{}];                  (* identity = non-scrambling ctrl *)
  ...
  sweep = CCTHPNoiseSweep[sc["gates"], {0.005, 0.010, 0.015, 0.020}, 300, 7];
  t2Noise = {};
...
Module[{tab, nPairs = 3, xx, yy, zz, sA, sHalf, pur},
  ...
  xx = CCTPairExp[tab, 1, 2, "X", "X"];
  yy = CCTPairExp[tab, 1, 2, "Y", "Y"];
  zz = CCTPairExp[tab, 1, 2, "Z", "Z"];
  sA    = CCTStabEntropy[tab, {1, 3, 5}];      (* one half of each pair *)
  sHalf = CCTStabEntropy[tab, {1}];
  pur   = CCTStabPurity[tab, {1, 2}];          (* joint pair purity (pure=1) *)
...
Module[{reps, pairs, bundle, tab, T, sOpt, cfTsC, cfTsLP, cf0C, cf0LP,
  ...
  FreeTableau[tab];
  cfTsC = CCTCFofS[2 Sqrt[2]]; cfTsLP = CCTCFofSLP[2 Sqrt[2]];
  cf0C = CCTCFofS[2]; cf0LP = CCTCFofSLP[2];
  cf225C = CCTCFofS[2.25]; cf225LP = CCTCFofSLP[2.25];
  tgtTs = N[Sqrt[2] - 1];
...
CCTConcurrence[rho_] := Module[{Y2, rhoTilde, R, ev, l},
...
CCTPairConcurrence[tab_Symbol, a_Integer, b_Integer] := Module[{rho},
  rho = CCTReducedRhoState[tab, {a, b}];
  rho = rho/Tr[rho];
  CCTConcurrence[rho]];
...
Module[{nPairs = 5, n, tab, ext, intr, extActive = {}, intActive = {},
  ...
  Do[Module[{},
     ...
     If[Length[extActive] >= 2, CCTScramble[tab, extActive, 1]];  (* mix exterior *)
     sInt  = CCTStabEntropy[tab, intActive];        (* interior block entropy *)
     cFirst = CCTPairConcurrence[tab, ext[1], intr[1]];  (* first pair concurrence *)
