# 00-BBT-blackbox-protocol — the primary module
...
- `mbqc_blackbox_test.py` — the full statistical certification protocol. Combines correlation-based classification of measurement statistics (contextual fraction / Global Exclusivity, from `01-D2-core-computation/`) with a Lie-algebraic / DLA resource-scaling hook (from the `LP-001` interface documented in `BlackBox/Kernel/BlackBox.wl`), across 7 pre-registered gates, to distinguish a genuine quantum KCBS device from classical (NCHV) and classical-optics "intensity-emulator" impostors. Has one documented, pre-declared exact blind spot (an intensity emulator tuned exactly to the quantum table is table-level indistinguishable by construction) — see `BBT-002` in the ledger.
...
- `01-D2-core-computation/` — the KCBS pentagon graph invariants, GE mechanism, and contextual-fraction machinery this protocol's statistical certificates are built from.
