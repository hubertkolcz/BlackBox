(* lever_sdp-decompose_e2e.wl -- END-TO-END byte-identity test for the reduced
   (equality-eliminated, "red2") joint-SDP formulation. This is the ORIGINAL
   GenerateEpsilonCertificate9.wl pipeline (all 4 seeds, converged-seed
   preference, Stage 2 exact repair, Stage 3 exact verification) with EXACTLY
   ONE change: SolveJoint solves the reduced formulation (11N Q/R entries
   eliminated via the exact local substitution + rvar Fourier-Motzkin) and then
   reconstructs the eliminated entries from the substitution rules, so
   downstream stages see a full-format solution. Exports NOTHING; prints the
   final GammaExact and byte-compares it against EpsilonCertificate_testK<K>_output.wl.
   Usage: wolframscript -file lever_sdp-decompose_e2e.wl <K>   (K in 3..5) *)

SetDirectory[DirectoryName[$InputFileName]];
K = ToExpression[$ScriptCommandLine[[-1]]];
MAXPOLICYROUNDS = 20;
RATIONALTOL = 10^-9;
Print["[E2E] K = ", K, " (reduced-formulation SolveJoint, everything else verbatim)"];
...
(* SolveJoint, reduced: solve red2, then reconstruct eliminated Q/R entries
   (exact affine images of the reduced solution) and rvar values (tight value
   rvar[e] = min_s of its three upper bounds -- any value in the feasible
   interval is equivalent; rvar is never consumed downstream) *)
SolveJoint[strategy_] := Module[{potConsR2, solR, elimSol, rvarSol},
   potConsR2 = Flatten[Table[
      ...
      {e, edges}, {s, 1, 3}]];
   solR = SemidefiniteOptimization[gammaVar, Join[psdConsRed, potConsR2], varsRed2];
   If[Head[solR] =!= List, Return[solR]];
   elimSol = Thread[elimVars -> (elimRHS /. solR)];
   rvarSol = Table[
     Module[{w = e[[1]], x = e[[2]], T},
       T = If[edgeLetter[e] === "d", Td, Tt];
       rVar[e] -> Min[Table[
          Module[{sig = strategy[{s, e}]},
           T[[s, sig]] + (phiVar[sig - 1, x] /. solR) - (phiVar[s - 1, w] /. solR)],
          {s, 3}]]],
     {e, edges}];
   Join[solR, elimSol, rvarSol]];
...
Print["Stage 1 (reduced SolveJoint), cap ", MAXPOLICYROUNDS, " rounds per seed..."];
t1 = AbsoluteTime[];
seedResults = {
