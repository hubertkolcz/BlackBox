...
of exact theta-bar over direct-frequency fc from enumerated primitive necklaces.
...
so gap-bound >= 3/2 - 4/3 = 1/6 = 0.1667 >> gap(ddt)=0.0699. Method cannot work."""
...
def main():
    ...
    print(f"global max theta-bar = {max_theta:.9f} at '{argmax}' (fc={argmax.count('d')/len(argmax):.4f})")
    # words achieving theta = 3/2
    at15 = sorted({(round(w.count('d')/len(w), 6), w) for w in words if abs(float(TH(w)) - 1.5) < 1e-6})
    ...
    print(f"ddt actual theta       = 1.4032309  (interior gap {h23-1.4032309:.7f} below hull)")
    print(f"floor at fc=2/3        = 1.3333333")
    print(f"=> Legendre gap-bound at 2/3 >= hull - floor = {h23 - 4.0/3.0:.7f}")
    print(f"   (flat Gamma_9 = 0.0720265 ; gap(ddt)=0.0698975)")
