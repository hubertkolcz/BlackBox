...
Note: `qutrit.wl` moved to `01-D2-core-computation/` — it's a shared primitive `kcbs.wl` also depends on, not mesh-specific.

## Relationship to the primary module

Feeds the primary module conceptually (the mesh gluing results are the reason a cluster-state analogue of the pentagon is worth building at all) but `00-BBT-blackbox-protocol/` does not import from here directly.
