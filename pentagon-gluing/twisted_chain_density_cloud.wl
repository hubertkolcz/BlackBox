(* twisted_chain_density_cloud.wl
   =============================================================================
   Wolfram Compute Services (RemoteBatchSubmit) script: twisted-chain bulk-density
   check at m = 200 .. 5000, upgrading ledger key D3_twistedChainNumerical toward
   the conjecture

       open twisted-chain bulk density  ->  tauStar
                                       =  Root[49 x^3 - 128 x^2 - 75 x + 218, 2]
                                       ~  1.3767177459158590533

   (tauStar is the PROVEN N->Infinity density of the twisted RING, CaseStudies.wl
   ...
   small-m boundary transients -- see the D3_twistedChainNumerical comment block.)

   SELF-CONTAINED: no Needs/Get/PacletDirectoryLoad. Everything needed is
   inlined below with attribution:
     - twistedChainWL[m]     : VERBATIM from pentagon-gluing/
                             twisted_chain_density_check.wl (identical to the
                             CaseStudies.wl Case-D3 "twisted-chain anchors" cell).
     ...
     - thetaSparseScaled[g]: LovaszThetaSparse[g, "Certificate"] from
         ...
         (2) RAISED SOLVER OPTIONS per the SemidefiniteOptimization::dfedge
             ...
             (twisted_chain_density_check.wl) was killed at m = 1000:
...
(* ------------------------------------------------------------------------- *)
(* twistedChainWL[m]: the open twisted chain, 3m+2 vertices.
   VERBATIM from twisted_chain_density_check.wl / CaseStudies.wl Case D3.        *)
(* ------------------------------------------------------------------------- *)
...
Do[
  Module[{msgs0, t, res, incr, dev, tag, mem, newMsgs, dfedgeQ},
    ...
    {t, res} = AbsoluteTiming[thetaSparseScaled[twistedChainWL[m]]];
...
<|
  "LedgerKey" -> "D3_twistedChainNumerical",
  "Conjecture" -> "open twisted-chain bulk density -> tauStar = Root[49x^3-128x^2-75x+218, 2]",
  "TauStar" -> tauStar,
