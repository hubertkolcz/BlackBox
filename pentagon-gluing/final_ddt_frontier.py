"""LANE 1 -- ddt global optimality via the certified Legendre theta-frontier.

Reads the exact de Bruijn-k certificate export (final_ddt_frontier_kK.txt):
...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else "final_ddt_frontier_k9.txt"
    ...
    # --- anchor cycles: ddt, all-c, all-t  (cyclemean of d over their dB-9 cycles) ---
    # node index by word: reconstruct words from isc? we only have isc; instead find
    # ddt/ddd/ttt cycles by following edges. Build word->idx not available; use structure:
    ...
    print(f"frontier(fc=0.000) = {frontier[0]:.8f}   (pure twisted theta tau*=1.3767177)")
    i23 = int(np.argmin(np.abs(fcs - 2.0 / 3.0)))
    print(f"frontier(fc=0.667) = {frontier[i23]:.8f}   (ddt theta = 1.4032309)")
    print(f"frontier(fc=1.000) = {frontier[-1]:.8f}   (pure direct theta = 1.5)")
    ...
    print(f"   gap(ddt) target        = 0.06989750")
...
def anchors_cyclemean(n, edges, df, isc, k):
    """Find the all-c, all-t, and (ddt)^* cycles in de Bruijn-k by following the
    ...
    for (u, v, isca, rf) in edges:
        adj[u].append((v, isca))
    out = []
    # all-c: a fixed point node whose only out-neighbour repeating 'd' returns to itself
    # find node with a self-loop where added letter c (period-1 cycle, all c)
    def find_periodic(pattern):
        # pattern like "d","t","ddt"; find a cycle whose added-letter sequence == pattern repeated
        pl = len(pattern)
        target = [1 if ch == "d" else 0 for ch in pattern]
        for start in range(n):
            ...
            if ok and node == start and seq == target:
                ...
                return cm, fcv
        return None
    for name, pat in (("all-d", "d"), ("all-t", "t"), ("ddt", "ddt")):
        r = find_periodic(pat)
        if r:
