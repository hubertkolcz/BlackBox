...
New module, added 2026-07-14. Purely a presentation layer: every figure here re-renders a fact that is already kernel-verified elsewhere in the repo (`pentagon-gluing` / GE-002 for the hypergraph, `certification-protocol` / G7 for the leaf-confinement sphere) — this module adds no new claims, only visualizations of existing ones, plus a restyle pass over a couple of pre-existing figures in other modules. Two matched themes run through everything: **Paper** (white background, muted ink, for embedding in the essay/PDF) and **Gallery** (dark cinematic background, for a standalone showcase).
...
Two other, older figures were restyled in place as part of this same visual-quality pass but **live in their own modules, not here**: `composition-optimality/orbit_spectrum_figure.wl` and `optical-synthesis/DemoBlueprints.wl`. They're cross-referenced from `VisualGallery.nb` rather than duplicated.
