# open-search-frontier

Ledger track `D1` — "theory frontier," explicitly scoped as **numerics only; this track never claims a proof**. Investigates whether Cabello's Global Exclusivity (GE) mechanism singles out the Lovász number $\theta(G)$ for every exclusivity graph $G$ — open in general.

## Contents

`HeptagonCatalysis.wl`, `beyond7_clique_search.py`, `beyond7_theorem_sweep.py` — the heptagon-catalysis theorem (`ERG-001`, `ERG-002`) and the still-open nonagon catalysis cell (`ERG-003`, compared in scale to Heule's Schur Number Five). `d1_ge_copies_sweep.wl`/`.py`, `d1_k3_activation.wl`, `d1_k3_graphs.py`, `d1_k3_maxclique.c`, `d1_k3_verify_witnesses.py` — copy-activation brackets for graphs beyond the pentagon (`GE-002`, `GE-003`). `d1-numerics-sweep-2026-07-11.md`, `d1-k3-brackets-2026-07-11.md` — working notes.

## Relationship to the primary module

This is a **side investigation**, not an input `certification-protocol/` depends on. It informs the project's broader argument about GE's scope and limits but does not feed the certification protocol directly.

## Ledger cross-reference

`GE-002`, `GE-003`, `GE-004`, `ERG-001`, `ERG-002`, `ERG-003` (track `D1`).
