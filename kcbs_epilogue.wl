...
(* ::Text:: *)
(*Reading. nu prices contextuality in negative probability; CF prices it in contextual mixture weight; on the n-cycle the exchange rate is the odd-cycle constant n - 1 = 2 alpha, apparently everywhere on the no-signalling polytope. Consistency anchors: at the Wright box, CF = 1 (strong contextuality) converts to nu = 1/(n - 1) \[LongDash] exactly the negative weight of the affine decomposition that writes the box over the deterministic vertices; and the quantum maximum's nu = (Sqrt[5] - 2)/2 is CF/4 = (2 Sqrt[5] - 4)/4. LITERATURE NOTE (adversarial novelty audit, 11-12 July 2026): the proof this note leaves open already exists, in a different literature. CF = (n - 1) nu for cyclic n-systems is Camillo-Cervantes' Theorem 2.1 (arXiv:2305.16574, 2023) in the Contextuality-by-Default tradition (Kujala-Dzhafarov 2019; Cervantes 2023), where nu's counterpart CNT3 is defined by exactly the same L1-minimal-signed-quasiprobability LP used here \[LongDash] it directly answers the open problem Abramsky-Barbosa-Mansfield's 2017 PRL itself flagged. This note's machine evidence (the wide sampling across 40 asymmetric quantum states and 23 non-signalling mixtures, 9 certified beyond the quantum set) independently corroborates that theorem rather than establishing a new one; see SignedNegativity.wl for the full citation and the correct scoping (the law is specific to cyclic n-systems, not universal across scenario types \[LongDash] CHSH/GHZ carry ratio 2, not n - 1).*)
...
(* ::Text:: *)
(*Where this leaves the thread. The flow note located the negativity (born at P, parked on the undetected mode); the ledger note showed the location is cut-relative and found the frame-free invariant nu; this epilogue prices the invariant (CF = (n - 1) nu, apparently polytope-wide), certifies that the phase-space witnesses can never out-violate the pentagon they accompany (topology without metric), and shows the channel layer is uniformly magic-rich, making the cascade's conservation a statement about the orbit alone. Open beyond this note: the analytic proof of the currency law (the machine says it is a theorem; the proof route via the complete n-cycle inequality set is unclaimed), and everything hardware-facing.*)

(* ::Section:: *)
(*References*)

(* ::Item:: *)
(*M. Araujo, M. T. Quintino, C. Budroni, M. Terra Cunha, A. Cabello, PRA 88, 022118 (2013) (all noncontextuality inequalities for the n-cycle scenario).*)
