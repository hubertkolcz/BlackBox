(* Generator SKELETON for the window-k=10 windowed transfer-SDP epsilon-certificate
   (would-be EpsilonCertificate10.wl).
   [ISSUE-020 RESOLVED 2026-07-13: this file was copied from the k=9 cloud template
   with K=10 but still exported symbol+file "EpsilonCertificate9", which would have
   silently overwritten the real k=9 certificate on a local run; all export targets
   below now say 10.]
   Follows the SAME construction as
   ...
   from-scratch reconstruction of a plausible construction pipeline.
     ...
     (d) The final exact-rational RATIONALIZE + integer-preserving REPAIR (Stage 2)
         ...
         posSigma / posCheck logic (so a produced EpsilonCertificate10 association
   ...
   Run with:  wolframscript -file GenerateEpsilonCertificate10_cloud.wl -print all
...
K = 10;                    (* window size; try K = 4 or 5 first as a smoke test *)
...
(* ------------------------------------------------------------------------- *)
(* STAGE 4: package into the EpsilonCertificate10 association, matching the exact
...
If[nodeEqOK && edgeEqOK && psdOK && pointwiseOK && gammaCrossCheckOK && finalConverged,
  EpsilonCertificate10 = <|
    ...
    |>;
  Export["EpsilonCertificate10.wl",
    "(* Rational epsilon-optimality certificate, window k = " <> ToString[K] <>
     ". Generated by GenerateEpsilonCertificate10_cloud.wl -- SEE THAT FILE'S HEADER for \
what is/isn't yet independently re-verified beyond the in-script Stage 3 checks. \
*)\nEpsilonCertificate10 = " <> ToString[EpsilonCertificate10, InputForm] <> ";\n",
    "Text"];
  Print["Wrote EpsilonCertificate10.wl -- Gamma_", K, " = ", GammaExact,
    " = ", N[GammaExact, 10]],
  Print["NOT written: exact re-verification did not fully pass (see Stage 3 above). ",
    ...
    "before trusting/exporting any Gamma_10 value."]
];
