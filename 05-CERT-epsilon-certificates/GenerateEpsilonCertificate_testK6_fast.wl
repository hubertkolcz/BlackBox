(* Generator SKELETON for the window-k=9 windowed transfer-SDP epsilon-certificate
   (would-be EpsilonCertificate6.wl), following the SAME construction as
   ...
   from-scratch reconstruction of a plausible construction pipeline.
     ...
     (d) The final exact-rational RATIONALIZE + integer-preserving REPAIR (Stage 2)
         ...
         posSigma / posCheck logic (so a produced EpsilonCertificate6 association
   ...
   Run with:  wolframscript -file GenerateEpsilonCertificate6.wl -print all
...
K = 6;                    (* window size; try K = 4 or 5 first as a smoke test *)
MAXPOLICYROUNDS = 10;      (* strategy-iteration cap; mean-payoff games converge
...
(* ------------------------------------------------------------------------- *)
(* STAGE 4: package into the EpsilonCertificate6 association, matching the exact
...
If[nodeEqOK && edgeEqOK && psdOK && pointwiseOK && gammaCrossCheckOK && finalConverged,
  EpsilonCertificate6 = <|
    ...
    |>;
  Export["EpsilonCertificate6.wl",
    "(* Rational epsilon-optimality certificate, window k = " <> ToString[K] <>
     ". Generated by GenerateEpsilonCertificate6.wl -- SEE THAT FILE'S HEADER for \
what is/isn't yet independently re-verified beyond the in-script Stage 3 checks. \
*)\nEpsilonCertificate6 = " <> ToString[EpsilonCertificate6, InputForm] <> ";\n",
    "Text"];
  Print["Wrote EpsilonCertificate6.wl -- Gamma_", K, " = ", GammaExact,
    " = ", N[GammaExact, 10]],
