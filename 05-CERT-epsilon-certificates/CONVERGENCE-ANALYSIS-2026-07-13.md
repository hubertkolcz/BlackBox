...
| k | Γ_k (certified upper bound) | ε_k = Γ_k − gap(cct) |
|---|---|---|
...
with gap(cct) = 0.0698975 the achieved lower bound. The sequence is **monotone
decreasing and converging toward gap(cct)** — exactly the behaviour the ε-certificate
hierarchy must have if cct is optimal. K=10 shrank the provable ε by ~27% (0.00213 →
...
|---|---|---|---|
| 0.5000040 (k=7 rnd), 0.500004 (k=10) | 1.500004 | **3/2** = pure-**cis** density (exact) | 4×10⁻⁶ |
| 0.3767230 (k=10) | 1.376723 | **τ\*** = pure-**trans** density (Root[49x³−128x²−75x+218]) | 5×10⁻⁶ |
...
| 0.29, 0.16 (k=9) | 1.29, 1.16 | trans-heavy mixed orbits | — |
...
strategy corresponds to one periodic orbit; the two structural extremes are pure-cis
(density 3/2, the maximum) and pure-trans (density τ\*). The reported value is
...
`θ̄(orbit) − 1` instead of the true `θ̄(orbit) − ᾱ(orbit)`. The **true** optimum (cct,
gap ≈ 0.07) is reached only when Ψ develops the correct α-cocycle (potentials
(0, −½, −1); the α-cis theorem), which penalises every orbit down to its real gap.

So the "garbage" numbers are a **read-out of the periodic-orbit spectrum** of the
gluing dynamics, with the α-correction switched off — cis and trans appear first
because they are the extremal orbits; the 16/11, 19/13, 25/17 family are mixed orbits
with densities crowding the cis end.
...
- **Only the random seeds differ**, and they degrade *continuously* with k, not
  suddenly at k=10:
  - k=7: random seeds **converge** — to spurious orbits (cis 3/2−1, and a mixed 0.3945).
  - k=8: random seeds **stop converging** (oscillate around 16/11−1, 19/13−1).
  - k=10: random seeds **oscillate longer**, cycling cis(0.5) ↔ 25/17(0.47) ↔ trans(0.377).
...
- The Γ_k sequence and the monotone bracket → gap(cct). (Γ₁₀ is a real result.)
- The identification of the spurious values as periodic-orbit densities, with pure-cis
  (3/2) and pure-trans (τ\*) appearing as the extremal orbits — a clean, verifiable exhibit.
...
3. **Explains WHY the certificate cannot close ε — and connects it to the open problem.**
   ...
   orbit gaps form a dense set of *rationals* approaching the *irrational* gap(cct), so a
   finite-k rational Γ_k is always strictly above. The same crowding is why the cct-tilt
   route was killed and why global optimality of cct stays open (near-optimal, long-period
...
cis density 3/2 is exact (key D3_cisLawProven); τ\* is exact (key D3_densityClosedForm).
