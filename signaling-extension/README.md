...
`signaling_taxonomy.wl`/`.py` — the three-gate taxonomy verified on 7 exemplar models (`SIG-001`), the exact communication-cost identity for the quantum KCBS table (`SIG-002`), and a candidate identity refuted by random sampling (`SIG-003`). `NOTES-signaling.md` — working notes.
...
`SIG-001`, `SIG-002`, `SIG-003` (track `SIG`).
