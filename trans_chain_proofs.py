"""Rigorous, self-contained proofs for the open TRANS pentagon chain (Python-only,
no SDP solver, no Wolfram kernel). Closes two QUANTUM_CONTEXTUALITY.md sec.9 items:

  #1  alpha(open trans chain of m pentagons) = floor(4 (m+1) / 3)   for all m >= 1.
  #2  the trans-chain theta-density EXISTS (Fekete via a PROVEN subadditivity) and = tau*.
...
pair). With seed a0 = (0,1,1) and the trans transfer matrix
...
IDENTIFICATION L = tau*.  The bulk density of the trans RING is tau* (proven, KKT
...
tau*, so L = tau*.  Thus the trans-chain gap is EXTENSIVE, ~ (tau* - 4/3) m ~ 0.0434 m,
...
def theta_chain(m):
    """theta of the open trans chain of m pentagons (orthonormal-representation SDP)."""
