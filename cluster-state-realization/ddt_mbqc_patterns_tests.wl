...
(* ===========================================================================
   ddt_mbqc_patterns_tests.wl -- full validation suite for ddt_mbqc_patterns.wl.
   ...
   Run:  wolframscript -file ddt_mbqc_patterns_tests.wl
   =========================================================================== *)

DDTMBQCPatternsLoadOnly = True;
Get[FileNameJoin[{DirectoryName[$InputFileName], "ddt_mbqc_patterns.wl"}]];
...
Print["############################################################"];
Print["### ddt_mbqc_patterns.wl  VALIDATION SUITE"];
Print["############################################################"];
...
Module[{rho1, sname, res},
  ...
  res = Table[Module[{v = mGState[{{1, 2}}, 2], w},
      w = mProj[v, DDTPrepSpec[st][[1]], 1, 2, DDTPrepSpec[st][[2]]];
      sname[rho1[w, 2, 2]] === st], {st, {"0", "1", "+", "-", "+i", "-i"}}];
  check["all 6 DDTPrepSpec preparations leave the intended Bloch state on the head", And @@ res]];
Print[];
...
Module[{edges = DDTMeshEdges[1], n = 9, prep = 3, head = 2, others = {4, 5, 6, 7, 8, 9},
