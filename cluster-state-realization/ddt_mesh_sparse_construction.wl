...
(* ===========================================================================
   ddt_mesh_sparse_construction.wl -- O(L) pentagon-mesh EDGE-LIST construction
   to replace the O(L^2) wordRing[word,reps] from mesh-composition/CaseStudies.wl
   (also copied verbatim into ddt_cluster_stabilizer.wl /
   ...
   Run:  wolframscript -file ddt_mesh_sparse_construction.wl
   =========================================================================== *)

(* ---------------------------------------------------------------------------
   SECTION 0. ORIGINAL O(L^2) wordRing -- reproduced VERBATIM (byte-for-byte,
   from mesh-composition/CaseStudies.wl / ddt_cluster_stabilizer.wl) as the
...
(* ---------------------------------------------------------------------------
   ...
   word="ddt", reps = 1,2,3,5,10,50 (all still cheap under the O(L^2)
   ...
   --------------------------------------------------------------------------- *)
Print["=== SECTION 2: regression test, wordRingEdgesFast vs wordRingOriginal (word=\"ddt\") ==="];
regressionReps = {1, 2, 3, 5, 10, 50};
regressionResults = Table[
   Module[{gOrig, origEdges, origV, fastEdges, fastV, edgesMatch, vertexMatch},
     gOrig = wordRingOriginal["ddt", reps];
     origEdges = Sort[Sort /@ (List @@@ EdgeList[gOrig])];
     origV = VertexCount[gOrig];
     fastEdges = Sort[wordRingEdgesFast["ddt", reps]];
     fastV = 3 StringLength[StringRepeat["ddt", reps]];
     edgesMatch = (origEdges === fastEdges);
     vertexMatch = (origV === fastV);
     <|"reps" -> reps, "L_pentagons" -> StringLength[StringRepeat["ddt", reps]],
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
Print["=== SECTION 3: benchmark, wordRingEdgesFast[\"ddt\",reps] (edge-list only) ==="];
...
Do[
  Module[{L = 3 reps, res, t, nEdges, nVerts, timedOut},
    {t, res} = AbsoluteTiming[
       TimeConstrained[
         With[{e = wordRingEdgesFast["ddt", reps]}, {Length[e], 3 L}],
         300, "TIMED_OUT"]];
...
Do[
 Module[{L = 3 reps, edges, tEdges, tGraphDefault, tGraphNoLayout, vcount, gDefault, gNoLayout,
   ...
   {tEdges, edges} = AbsoluteTiming[wordRingEdgesFast["ddt", reps]];
   vcount = 3 L;
