...
(* ===========================================================================
   cct_mbqc_hawking_certification_tests.wl -- FULL-SCALE certification runner
   for cct_mbqc_hawking_certification.wl.  Executes all five stages at scale,
   ...
   SCALES (headline):
     stage 1  M = 100 disjoint mesh-carved Hawking pairs @ reps = 1000
              (9000-qubit "cct" mesh), each verified to be the exact 2 sqrt2
...
CCTHawkingCertLoadOnly = True;
Get[FileNameJoin[{DirectoryName[$InputFileName], "cct_mbqc_hawking_certification.wl"}]];

Print["==================================================================="];
Print[" cct_mbqc HAWKING-PAIR CHSH/CF CERTIFICATION -- FULL-SCALE RUNNER"];
Print["==================================================================="];
Print["pre-registered budget: alpha=", N[CCTHawkAlpha], " / ", CCTHawkNTests,
  " tests -> per-test delta=", CCTHawkDelta];
Print[];
...
Module[{reps = 1, pairs, bundle, tab, allOK},
  pairs = CCTHawkingSelectPairs[reps, 2];
  bundle = CCTHawkingBuildCarved[reps, pairs, "Forced" -> 0]; tab = bundle["tab"];
  allOK = AllTrue[pairs, Function[p, Module[
    {a = p["partner"], b = p["hawking"], rdmE, v, nn = CCTMeshN[reps], T2, other, rdmSV},
    rdmE = CCTExactRDM[tab, a, b];
  ...
  Print["  1a  exact-RDM == StateVectorFromTableau partial trace (reps=1, ",
    Length[pairs], " pair(s), n=", CCTMeshN[reps], "): ", allOK];
...
Module[{reps = 1000, M = 100, pairs, bundle, tab, tsel, tbuild, chsh, allBell,
   allDisjoint, used},
  {tsel, pairs} = AbsoluteTiming[CCTHawkingSelectPairs[reps, M]];
  used = Flatten[{#["partner"], #["hawking"], #["carve"]} & /@ pairs];
  allDisjoint = (Length[used] === Length[Union[used]]);   (* disjoint regions *)
  {tbuild, bundle} = AbsoluteTiming[CCTHawkingBuildCarved[reps, pairs, "Forced" -> 0]];
  tab = bundle["tab"];
  chsh = Table[CCTCHSHOptimal[CCTPairCorrMatrix[tab, p["partner"], p["hawking"]]], {p, pairs}];
  allBell = AllTrue[chsh, Abs[# - 2 Sqrt[2]] < 10^-9 &];
  FreeTableau[tab];
  Print["  1b  M=", Length[pairs], " disjoint pairs @ reps=", reps,
    " (n=", CCTMeshN[reps], " qubits); select=", tsel, "s build+carve=", tbuild, "s"];
...
Module[{reps = 100, pairs, builds = 68, nd},
  pairs = CCTHawkingSelectPairs[reps, 10^6];
  SeedRandom[20260713];
  nd = CCTHawkingNoDisturbance[reps, pairs, builds];
  ...
  Print["  Hoeffding eps (delta=", CCTHawkDelta, ") = ", nd["eps"],
...
Module[{reps = 100, pairs, bps = 30, tomo, exact, est, devs, maxdev, pass, recRDM,
  ...
  Module[{bundle = CCTHawkingBuildCarved[reps, pairs, "Forced" -> 0]},
    exRDM = CCTExactRDM[bundle["tab"], pairs[[1]]["partner"], pairs[[1]]["hawking"]];
    exact = Association[Table[pl ->
       Re[Tr[ConjugateTranspose[KroneckerProduct[CCTHawkPauliMat[pl[[1]]], CCTHawkPauliMat[pl[[2]]]]] . exRDM]],
       {pl, CCTHawk15}]];
    FreeTableau[bundle["tab"]]];
  SeedRandom[20260714];
  tomo = CCTHawkingTomography[reps, pairs, bps];
  est = tomo["est"];
  devs = Association[Table[pl -> Abs[Lookup[est, Key[pl], 0] - exact[pl]], {pl, CCTHawk15}]];
  maxdev = Max[Values[devs]];
  pass = (maxdev <= tomo["eps"]);
  recRDM = CCTReconstructRDM[est];
...
Module[{reps = 3, pairs, bundle, tab, T, sOpt, sExp, cfTsC, cfLocC, cf225C,
  ...
  pairs = CCTHawkingSelectPairs[reps, 1];
  bundle = CCTHawkingBuildCarved[reps, pairs, "Forced" -> 0]; tab = bundle["tab"];
  T = CCTPairCorrMatrix[tab, pairs[[1]]["partner"], pairs[[1]]["hawking"]];
  FreeTableau[tab];
  ...
  cfTsC = CCTCFofS[2 Sqrt[2]]; cfLocC = CCTCFofS[2]; cf225C = CCTCFofS[9/4];
  cfTsLP = CCTCFofSLP[2 Sqrt[2]]; cfLocLP = CCTCFofSLP[2]; cf225LP = CCTCFofSLP[9/4];
  ...
  Print["  => certified Hawking-pair CHSH S=", N[sOpt],
    " lands at CF=", N[CCTCFofS[sOpt]], " = sqrt2-1 (Tsirelson ceiling)"];
  (* monotone + closed-form check of the LP over a grid *)
  mono = Module[{cfs = Table[CCTCFofSLP[s], {s, 2., 4., 0.1}]},
     AllTrue[Differences[cfs], # >= -10^-9 &]];
  linform = Max[Table[Abs[CCTCFofSLP[s] - (s - 2)/2], {s, 2., 4., 0.1}]];
...
Module[{reps = 3, pairs, ps, K = 4000, curve, pDeath, p225, mcOK, deathBracket,
  ...
  SeedRandom[20260715];
  curve = CCTHawkNoiseCurve[reps, pairs[[1]], ps, K];
  ...
  (* exact crossings *)
  pDeath = CCTHawkPForCHSH[2];        (* violation death: CHSH=2 *)
  p225 = CCTHawkPForCHSH[2.25];       (* CHSH=2.25 landing, CF=0.125 *)
  Print["  EXACT violation-death crossing  CHSH(p)=2     at p = ", N[pDeath],
    " = 1-1/sqrt2"];
  Print["  EXACT CHSH(p)=2.25 crossing (S=2.25 -> CF=0.125) at p = ", N[p225]];
  Print["  check CF at those crossings: CF(2)=", N[CCTCFofS[2]],
    "  CF(2.25)=", N[CCTCFofS[2.25]]];
