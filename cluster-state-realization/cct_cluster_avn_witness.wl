...
(* ===========================================================================
   cct_cluster_avn_witness.wl -- generalizing mbqc_c5.wl's single-pentagon GHZ
   All-vs-Nothing (AvN) contextuality witness to the cct-glued pentagon-MESH
   ...
   THREE FRAMEWORKS BRIDGED (all read in full before writing this file):
     ...
     * mesh-composition/CaseStudies.wl -- wordRing[word,reps], the pentagon
       ...
       file cct_cluster_stabilizer.wl already did for the binary GF(2)
       stabilizer tableau). That file's wordRing["cct",reps] RING is the
       literal "cct-glued mesh" this task names.
   ...
   HEADLINE RESULT (verified below, not merely argued):
     ...
     pentagons (chain and the literal cct RING), and cross-validated against
   ...
   Run:  wolframscript -file cct_cluster_avn_witness.wl
   =========================================================================== *)

Print["=== cct_cluster_avn_witness.wl : mesh-generalized GHZ AvN witness ==="];
Print[];
...
(* ---------------------------------------------------------------------------
   SECTION 5. The literal "cct-glued" mesh: wordRing["cct",1], a RING of 3
   pentagons (9 qubits). Graph combinatorics reused VERBATIM from
   CaseStudies.wl / cct_cluster_stabilizer.wl, reinterpreted as a CZ/qubit
   graph exactly as that sibling file already established.
   --------------------------------------------------------------------------- *)
Print["--- Section 5: literal cct-glued RING, wordRing[\"cct\",1] (9 qubits) ---"];
...
Print["N=3 cct RING wordRing[\"cct\",1] (9 qubits, the literal named object, every block interior):",
