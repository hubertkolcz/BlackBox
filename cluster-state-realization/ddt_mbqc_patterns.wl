...
(* ===========================================================================
   ddt_mbqc_patterns.wl -- quantum-algorithm MEASUREMENT PATTERNS on the fixed
   pentagon-mesh graph state, executed through the sparse CHP stabilizer
   simulator ddt_mbqc_sim.wl.  Library of pattern functions:
   ...
   PHYSICS CONVENTIONS (fixed project-wide, matching mbqc_c5.wl and the sim):
     ...
     * The quantum resource is the FIXED mesh graph state from
       wordRingEdgesFast["ddt",reps]; the edge list is NEVER edited.  Unused
   ...
   KNOWN LIMITATIONS (review 2026-07-13, documented as accepted):
     * TeleportWire (Gadget 1 ONLY) POST-SELECTS the input-injection
       ...
       forced to the DDTPrepSpec canonical value (probability 1/2 per shot on
     ...
     * The Grover cluster pattern is calibrated to the k == 0 (mod 3) pentagon
       context of the period-3 "ddt" word (see Section P4 note).

   DEPENDS ON:  ddt_mbqc_sim.wl  (same directory).  Set DDTMBQCPatternsLoadOnly
   = True before Get to suppress the self-check.

   Run:  wolframscript -file ddt_mbqc_patterns.wl   (executes a fast self-check)
   Full validation suite:  ddt_mbqc_patterns_tests.wl
   =========================================================================== *)

If[!ValueQ[DDTMBQCPatternsDir],
  DDTMBQCPatternsDir = DirectoryName[$InputFileName]];
Block[{DDTMBQCLoadOnly = True},
  Get[FileNameJoin[{DDTMBQCPatternsDir, "ddt_mbqc_sim.wl"}]]];

(* ---------------------------------------------------------------------------
   SECTION P0. Mesh construction (wordRingEdgesFast copied VERBATIM from
   ddt_mesh_sparse_construction.wl -- O(L) edge list, verified there to equal
...
DDTMeshEdges[reps_Integer] := wordRingEdgesFast["ddt", reps];
DDTMeshN[reps_Integer] := 9 reps;

(* adjacency in the ORIGINAL (uncarved) mesh, from the edge list *)
DDTNbrs[edges_List, v_Integer] :=
  Union[Cases[edges, {v, u_} :> u], Cases[edges, {u_, v} :> u]];
DDTAdjacency[edges_List, n_Integer] := Module[{a = ConstantArray[{}, n]},
  Do[a[[e[[1]]]] = Append[a[[e[[1]]]], e[[2]]];
     a[[e[[2]]]] = Append[a[[e[[2]]]], e[[1]]], {e, edges}];
  Sort /@ a];
DDTTips[edges_List, n_Integer] := Module[{a = DDTAdjacency[edges, n]},
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
DDTketPsi["0"] = {1, 0};            DDTketPsi["1"] = {0, 1};
DDTketPsi["+"] = {1, 1}/Sqrt[2];    DDTketPsi["-"] = {1, -1}/Sqrt[2];
DDTketPsi["+i"] = {1, I}/Sqrt[2];   DDTketPsi["-i"] = {1, -I}/Sqrt[2];
DDTHmat = {{1, 1}, {1, -1}}/Sqrt[2];
DDTPmat["I"] = IdentityMatrix[2]; DDTPmat["X"] = {{0, 1}, {1, 0}};
DDTPmat["Z"] = {{1, 0}, {0, -1}}; DDTPmat["Y"] = {{0, -I}, {I, 0}};
...
DDTPrepSpec["0"] = {"X", 0};  DDTPrepSpec["1"] = {"X", 1};
DDTPrepSpec["+"] = {"Z", 0};  DDTPrepSpec["-"] = {"Z", 1};
DDTPrepSpec["-i"] = {"Y", 0}; DDTPrepSpec["+i"] = {"Y", 1};

(* reduced 1-qubit density of qubit q from a full statevector v (n qubits) *)
DDTReducedRho1[v_List, q_Integer, n_Integer] := Module[
...
DDTIdentifyByproduct[rhoOut_, psiExpected_List] := Module[{cands, rho, rn, tr},
...
(* ---------------------------------------------------------------------------
   ...
   injected by measuring the degree-1 prep vertex in the basis of DDTPrepSpec
   (a genuine single-qubit measurement -- see mapping above).  CAVEAT: the
   prep-vertex OUTCOME is post-selected to the DDTPrepSpec canonical value
...
TeleportWire[reps_Integer, path_List, inState_String, OptionsPattern[]] := Module[
  {edges = DDTMeshEdges[reps], n = DDTMeshN[reps], prep, out, interior, others,
  ...
  {prepBasis, prepForced} = DDTPrepSpec[inState];
  (* forced-outcome bookkeeping *)
  If[forced === Automatic,
    xForced = ConstantArray[Automatic, kH],
    DDTAssert[Length[forced] === kH + 1,
  ...
  frame = If[Head[vT] === List,
    rho = DDTReducedRho1[vT, out, n];
    DDTIdentifyByproduct[rho, MatrixPower[DDTHmat, kH] . DDTketPsi[inState]],
  ...
  <|"reps" -> reps, "path" -> path, "inState" -> inState, "kH" -> kH,
    ...
    "expectedLogical" -> MatrixPower[DDTHmat, kH] . DDTketPsi[inState]|>];
...
(* ---------------------------------------------------------------------------
   ...
   sim of wordRingEdgesFast["ddt",reps].  Length[secret] <= #tips = 3*reps.
   --------------------------------------------------------------------------- *)
RunBernsteinVazirani[reps_Integer, secret_List] := Module[
  {edges = DDTMeshEdges[reps], n = DDTMeshN[reps], tips, allTips, others, tab,
   zout, frame, raw, corrected, nbrsMeasured},
  allTips = DDTTips[edges, n];
  DDTAssert[Length[secret] <= Length[allTips],
    "RunBernsteinVazirani: secret longer than #tips (=3 reps)", {Length[secret], Length[allTips]}];
  DDTAssert[SubsetQ[{0, 1}, Union[secret]] || Union[secret] === {} ||
  ...
  frame = Table[
    nbrsMeasured = Select[DDTNbrs[edges, tips[[i]]], KeyExistsQ[zout, #] &];
...
(* ---------------------------------------------------------------------------
   ...
   sim AND the explicit Grover unitary (see ddt_mbqc_patterns_tests.wl).
   ...
   --------------------------------------------------------------------------- *)
DDTGroverMeasCanon = {2, 4, 5, 6, 7, 8};
DDTGroverOutCanon = {1, 3};
DDTGroverPattern = <|
...
DDTGroverMarkInt[m_Integer] := m;
DDTGroverMarkInt[m_List] := FromDigits[m, 2];
...
DDTGroverRunInstance[tab_Symbol, edges_List, k_Integer, mark_Integer, zout_] := Module[
  {cm = DDTGroverCanonMap[k], bases, lin, meas, outs, pflip, clean, zread, frame},
  {bases, lin} = DDTGroverPattern[mark];
  meas = cm /@ DDTGroverMeasCanon; outs = cm /@ DDTGroverOutCanon;
  pflip[v_] := Mod[Total[Lookup[zout, Select[DDTNbrs[edges, v], KeyExistsQ[zout, #] &], 0]], 2];
...
RunGrover2[markedItem_, OptionsPattern[]] := Module[
  {mark = DDTGroverMarkInt[markedItem], reps = OptionValue["reps"], k = OptionValue["k"],
   edges, n, cm, cluster, others, tab, zout, corr},
  DDTAssert[Mod[k, 3] == 0,
    "RunGrover2: k must be == 0 (mod 3) to match the calibrated pentagon context", k];
  DDTAssert[2 <= k <= 3 reps - 2,
    "RunGrover2: need an interior pentagon pair (2 <= k <= 3 reps - 2)", {k, reps}];
  edges = DDTMeshEdges[reps]; n = DDTMeshN[reps];
  cm = DDTGroverCanonMap[k]; cluster = Values[cm];
  others = Complement[Range[n], cluster];
  tab = NewGraphStateTableau[n, edges];
  zout = Association[Table[q -> MeasurePauli[tab, q, "Z"]["Outcome"], {q, others}]];
  corr = DDTGroverRunInstance[tab, edges, k, mark, zout];
  FreeTableau[tab];
...
(* NB: the "ddt" word makes the pentagon-pair context period-3 in k; the fixed
...
RunGroverParallel[M_Integer, OptionsPattern[]] := Module[
  ...
  clusters = Values[DDTGroverCanonMap[#]] & /@ ks;
  allCluster = Union @@ clusters;
  others = Complement[Range[n], allCluster];
  tab = NewGraphStateTableau[n, edges];
  zout = Association[Table[q -> MeasurePauli[tab, q, "Z"]["Outcome"], {q, others}]];
  results = Table[Module[{corr = DDTGroverRunInstance[tab, edges, ks[[j]], marks[[j]], zout]},
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
If[!TrueQ[DDTMBQCPatternsLoadOnly],
  Module[{w, bv},
    Print["=== ddt_mbqc_patterns.wl self-check ==="];
    ...
    Print["DDTMBQCPatternsSelfCheck done."];
  ]
]
