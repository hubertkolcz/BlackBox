...
(* ===========================================================================
   ddt_mbqc_scale_run.wl -- SCALE RUNS of the MBQC measurement patterns on the
   fixed pentagon-mesh graph state, through the sparse CHP tableau simulator.
     ...
     * Contextual-NAND (Anders-Browne OR) throughput on mesh-carved GHZ
       ...
       exhaustively verified ddt_mbqc_contextual_nand.wl and re-validated
   ...
   SCALE-SPECIFIC IMPLEMENTATION NOTES (why not just call the library fns):
     * RunBernsteinVazirani / DDTGroverRunInstance in ddt_mbqc_patterns.wl
       compute byproduct frames via DDTNbrs[edges, v], an O(|E|) scan PER
       ...
       pattern table DDTGroverPattern is reused verbatim), and each scale
   ...
   DEPENDS ON: ddt_mbqc_patterns.wl (-> ddt_mbqc_sim.wl), same directory.

   Run:
     wolframscript -file ddt_mbqc_scale_run.wl validate   (validation only)
     wolframscript -file ddt_mbqc_scale_run.wl full       (default: validate
                       + BV 1e2..1e5 + Grover 1e3 + NAND throughput + table)
     wolframscript -file ddt_mbqc_scale_run.wl mega       (BV 1e6, Grover 1e4,
                       NAND 1e5 -- large-scale benchmark stage)
   =========================================================================== *)

If[!ValueQ[DDTScaleDir], DDTScaleDir = DirectoryName[$InputFileName]];
Block[{DDTMBQCPatternsLoadOnly = True, DDTMBQCLoadOnly = True},
  Get[FileNameJoin[{DDTScaleDir, "ddt_mbqc_patterns.wl"}]]];
...
DDTResults = {};   (* rows of the final consolidated table *)

(* ---------------------------------------------------------------------------
   SECTION 1. Bernstein-Vazirani at scale.
   Protocol identical to RunBernsteinVazirani (ddt_mbqc_patterns.wl):
   ...
   --------------------------------------------------------------------------- *)
DDTScaleBV[nSecret_Integer] := Module[
  ...
  {tE, edges} = AbsoluteTiming[DDTMeshEdges[reps]];
  {tT, tab} = AbsoluteTiming[
     NewGraphStateTableau[n, edges, "ValidateEdges" -> (n <= 10^5)]];
  adj = tab["adj"];
  tips = Range[3, 3 nSecret, 3];          (* == Take[DDTTips[...], nSecret], checked in validation *)
  tipDegOK = Union[Length /@ adj[[tips]]] === {2};
  tipNbrOK = FreeQ[Mod[Flatten[adj[[tips]]], 3], 0];  (* no tip adjacent to a tip *)
  DDTAssert[tipDegOK && tipNbrOK, "DDTScaleBV: tip structure violated", {nSecret, tipDegOK, tipNbrOK}];
...
(* ---------------------------------------------------------------------------
   ...
   Geometry, canonical pattern table (DDTGroverPattern), byproduct rules and
   marks handling identical to RunGroverParallel; frame parities via adjacency.
   --------------------------------------------------------------------------- *)
DDTScaleGrover[M_Integer] := Module[
...
(* ---------------------------------------------------------------------------
   ...
   Settings/signs taken from ddt_mbqc_contextual_nand.wl (exhaustively
   ...
   --------------------------------------------------------------------------- *)
DDTORGadget[tab_Symbol, {s1_, s2_, s3_}, c_List, a_Integer, b_Integer] :=
...
DDTScaleNAND[M_Integer] := Module[
  ...
  structOK = AllTrue[triples, Function[tr,
     ...
     AllTrue[Complement[Flatten[adj[[tr]]], tr],
       !MemberQ[{1, 2, 3}, Mod[#, 9]] &]]];
  DDTAssert[structOK, "DDTScaleNAND: triple structure violated", M];
  ...
  {tAlg, outs} = AbsoluteTiming[Table[
     Module[{tr = triples[[j]], c},
      ...
      DDTORGadget[tab, tr, c, 1 - xs[[j]], 1 - ys[[j]]]], {j, M}]];
...
DDTRunValidation[] := Module[
  ...
  (* V1: tip formula == DDTTips *)
  edges5 = DDTMeshEdges[5];
  check["V1: tips = Range[3,9reps,3] == DDTTips (reps=5)",
    Range[3, 45, 3] === DDTTips[edges5, 45]];
  ...
  check["V2a: library RunBernsteinVazirani reps=4 recovers planted secret",
    bvLib["correct"]];
  bvScale = DDTScaleBV[12];
  check["V2b: DDTScaleBV n=12 recovers planted secret", bvScale["correct"]];
  bvScale = DDTScaleBV[100];
  check["V2c: DDTScaleBV n=100 recovers planted secret", bvScale["correct"]];
  ...
  gpScale = DDTScaleGrover[10];
  check["V3b: DDTScaleGrover M=10 all correct (" <>
  ...
  If[!orOK,
    Print["*** SIGN DISAGREEMENT between ddt_mbqc_contextual_nand.wl settings"];
  ...
  If[orOK,
    ...
    multiOK = And @@ Flatten[Table[
       Module[{tab, adj, zarr, others, out, tr},
        tab = NewGraphStateTableau[54, DDTMeshEdges[6]];
        adj = tab["adj"];
    ...
    r = DDTScaleNAND[20];
    check["V6: DDTScaleNAND M=20 all NAND outputs match truth table (" <>
...
DDTAddRow[res_Association, memMB_] := AppendTo[DDTResults,
...
DDTPrintTable[] := Module[{cols, widths, rows, hdr},
  ...
  rows = Table[ToString /@ {r["task"], r["size"], r["qubits"], r["meas"],
      ...
      fmt[r["tTotal"]], fmt[r["MB"]], r["maxW"], r["ok"]}, {r, DDTResults}];
  ...
  Module[{bv = Select[DDTResults, #["task"] === "BV" &]},
    If[Length[bv] >= 2,
      Print[];
...
DDTRunAndRow[label_String, f_] := Module[{res, mem},
  Print["--- ", label, " ---"];
  mem = MaxMemoryUsed[res = f[]];
  DDTAddRow[res, mem/2.^20];
...
Print["ddt_mbqc_scale_run.wl  stage = ", mode];
Print[];

valOK = DDTRunValidation[];
Print[];
...
Which[
  ...
  mode === "full",
  If[valOK,
    Do[DDTRunAndRow["Bernstein-Vazirani n=" <> ToString[nn],
       With[{n2 = nn}, DDTScaleBV[n2] &]], {nn, {100, 1000, 10000, 100000}}];
    DDTRunAndRow["Grover parallel M=1000", DDTScaleGrover[1000] &];
    DDTRunAndRow["Contextual NAND M=10000 (one gate per carved GHZ triple)",
      DDTScaleNAND[10000] &];
    Print[];
    DDTPrintTable[],
  ...
  mode === "mega",
  If[valOK,
    DDTRunAndRow["Grover parallel M=10000", DDTScaleGrover[10000] &];
    DDTRunAndRow["Contextual NAND M=30000", DDTScaleNAND[30000] &];
    Print[];
    DDTPrintTable[],
    Print["*** VALIDATION FAILED -- scale runs skipped."]],
