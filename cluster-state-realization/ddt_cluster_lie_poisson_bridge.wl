...
(* ===========================================================================
   ddt_cluster_lie_poisson_bridge.wl
   ...
   components for the pentagon-mesh MBQC / Lie-Poisson project:
     ...
     (1) ddt_cluster_stabilizer.wl   -- efficient GF(2) binary stabilizer
         tableau for the ddt-glued pentagon-mesh CZ cluster state. POLYNOMIAL
         in n (rank + symplectic-product on an n x 2n binary matrix); verified
         to N=100 pentagons (~300 qubits) in the source file.
     (2) ddt_cluster_avn_witness.wl  -- generalized GHZ all-versus-nothing (AvN)
         ...
         and the literal ddt ring wordRing["ddt",1] (9 qubits) in the source
         file.
     (3) ddt_cluster_dla.wl          -- Dynamical Lie Algebra dimension
   ...
   PARAMETERS below), and runs all three checks against the SAME ddt-glued
   ...
   validity radius in N, and this script does not pretend otherwise:
     ...
     * STABILIZER CHECK (Section 2): [UPDATED] mesh construction (Section 1,
       ...
       re-architected to be sparse/O(n+edges) -- see ddt_mesh_sparse_construction.wl
       and ddt_mesh_sparse_stabilizer.wl. Analytically, independence (rank=n)
     ...
     * AvN / CONTEXTUALITY WITNESS CHECK (Section 3): this is gated by
       ...
       ddt_cluster_avn_witness.wl). Beyond that qubit count, THIS SCRIPT
     ...
     * DLA DIMENSION CHECK (Section 4): gated even more tightly. Only the
       ...
       Section 4's closing remarks, reproduced from ddt_cluster_dla.wl
   ...
   Run locally:  wolframscript -file ddt_cluster_lie_poisson_bridge.wl
   =========================================================================== *)

(* ---------------------------------------------------------------------------
   PARAMETERS -- the only knobs you should need to touch.
   --------------------------------------------------------------------------- *)
nPentagonsRequested = 3;      (* <-- CHANGE THIS to scale N. 3 = the literal
                                  "ddt-glued" ring wordRing["ddt",1] (9 qubits),
                                  already the richest case validated end-to-end
                                  in ddt_cluster_avn_witness.wl. *)
avnMaxQubits = 11;             (* AvN check gate: largest qubit count actually
                                  verified in ddt_cluster_avn_witness.wl
...
denseCrossCheckMaxQubits = 5000; (* Stabilizer check gate (NEW): above this
                                  ...
                                  9000 qubits in ddt_mesh_sparse_stabilizer.wl,
...
reps = Max[1, Round[nPentagonsRequested/3]];  (* wordRing convention: actual
                                  ...
                                  exactly as ddt_cluster_stabilizer.wl uses it. *)

Print["=== ddt_cluster_lie_poisson_bridge.wl : integrated pentagon-mesh check ==="];
...
If[3 reps != nPentagonsRequested,
  Print["  NOTE: actual pentagon count (", 3 reps,
    ...
    ") -- wordRing[\"ddt\",reps] only produces multiples of 3, per its own convention."]];
Print[];

(* ---------------------------------------------------------------------------
   SECTION 0. Shared Pauli/Kronecker conventions -- verbatim and IDENTICAL
   across all three source files (mbqc_c5.wl, ddt_cluster_stabilizer.wl,
   ddt_cluster_avn_witness.wl, ddt_cluster_dla.wl), consolidated once here so
...
(* ---------------------------------------------------------------------------
   ...
   (Case D3, "The Optimal Gluing Word") via ddt_cluster_stabilizer.wl /
   ddt_cluster_avn_witness.wl: a RING of L = Length[word]*reps pentagons glued
   edge-to-edge (direct/twisted read off the repeated word), 3L vertices/qubits.
   Self-contained -- no paclet dependency, so this works identically on a bare
   cloud kernel.

   PentagonChainEdges[nblocks] -- REUSED VERBATIM from ddt_cluster_avn_witness.wl
...
(* SPARSE REWRITE (this update): the ORIGINAL wordRing built its edge list via
   ...
   ddt_mesh_sparse_construction.wl): reps=10000 -> 11.5s for construction
   ...
   word="ddt", reps=1,2,3,5,10,50 in ddt_mesh_sparse_construction.wl Section 2
...
meshEdges = wordRing["ddt", reps];
nQ = 9 reps;
actualPentagons = 3 reps;
Print["Mesh built: wordRing[\"ddt\",", reps, "] -- ", actualPentagons,
  " pentagons, ", nQ, " qubits, ", Length[meshEdges], " CZ edges."];
Print[];

(* ===========================================================================
   SECTION 2. STABILIZER VALIDITY CHECK (POLYNOMIAL, scales to large N).
   REUSED, essentially verbatim, from ddt_cluster_stabilizer.wl.
   =========================================================================== *)
...
(* independence (rank over GF(2)) + mutual commutation (binary symplectic
   ...
   57.5s for just 9000 qubits in ddt_mesh_sparse_stabilizer.wl. KEPT for the
...
(* ---------------------------------------------------------------------------
   ...
   (see ddt_mesh_sparse_stabilizer.wl for the full derivation):
...
If[denseCrossCheckAttempted,
  ...
  (
   ...
   Print["  dense cross-check SKIPPED: nQ=", nQ, " exceeds denseCrossCheckMaxQubits=",
     ...
     " method exists to avoid; see ddt_mesh_sparse_stabilizer.wl)."];
  )];
...
Print["  STABILIZER CHECK PASSED? ", stabPassed,
  ...
  " 9,000,000 pentagons in ~312s in ddt_mesh_sparse_stabilizer.wl -- genuinely",
  " scales to millions of pentagons, unlike the old dense method)."];
Print[];

(* ===========================================================================
   SECTION 3. AvN / CONTEXTUALITY WITNESS CHECK (EXPONENTIAL, gated).
   REUSED, essentially verbatim, from ddt_cluster_avn_witness.wl.
   =========================================================================== *)
...
If[avnAttempted,
  ...
  (
   ...
   Print["  SKIPPED: nQ=", nQ, " exceeds avnMaxQubits=", avnMaxQubits,
     " (the largest qubit count actually verified in ddt_cluster_avn_witness.wl)."];
...
(* ===========================================================================
   ...
   REUSED, essentially verbatim, from ddt_cluster_dla.wl.
   =========================================================================== *)
...
(* GenerateDLAFast -- verified-equivalent, algorithmically faster closure:
   ...
   su(2^n) dimension cap 4^n-1. REUSED VERBATIM from ddt_cluster_dla.wl. *)
...
(* FIXED CEILING DEMONSTRATION: N=1 pentagon (5 qubits), the largest mesh size
   where the exact DLA closure is known (from ddt_cluster_dla.wl) to complete.
...
If[! dlaCeilingConfirmed,
  Print["  NOTE: ceiling computation did not complete/confirm within ", dlaCeilingTimeoutSeconds,
    "s on THIS machine -- relying on the previously-established result in ddt_cluster_dla.wl",
...
Which[
  ...
  dlaRequestedStatus == "BOUNDED_PROBE_INCOMPLETE",
    Module[{cap = 4^nQ - 1, probe, ptime},
      ...
      Print["  demonstrates infeasibility empirically, per ddt_cluster_dla.wl Section 7/8."];
  ...
  True,
    ...
    Module[{capDigits = Ceiling[N[nQ*Log10[4]]]},
      ...
      Print["  Per ddt_cluster_dla.wl Section 8: this is not a slow computation to speed up;"];
...
honestGaps = Join[
   If[! avnAttempted,
     {"AvN witness check SKIPPED at N=" <> ToString[actualPentagons] <>
       ...
       ToString[avnMaxQubits] <> " qubits established in ddt_cluster_avn_witness.wl."},
     {}],
   ...
   If[dlaRequestedStatus != "EXACT",
     {"DLA dimension at the REQUESTED N (" <> ToString[nQ] <> " qubits) is " <>
       ...
       "exactly established; see ddt_cluster_dla.wl for the full feasibility argument."},
     {}]
];
...
Print["ddt_mesh_sparse_stabilizer.wl / ddt_mesh_sparse_construction.wl); the AvN and DLA"];
...
<|
  ...
  "Stabilizer" -> <|
     ...
     "ScalesToLargeN" -> True,
     "ScaleEvidence" -> "benchmarked to 27,000,000 qubits / 9,000,000 pentagons in ~312s in ddt_mesh_sparse_stabilizer.wl; construction (Section 1) benchmarked to the same scale in ddt_mesh_sparse_construction.wl"
  |>,
  "AvN" -> <|
     ...
     (* FIXED (adversarial review): this field previously implied 8/11-qubit
        ...
        standalone ddt_cluster_avn_witness.wl, on a DIFFERENT topology (open
        PentagonChain, not this script's wordRing["ddt",reps] ring). The ONE
        ...
        (wordRing["ddt",1], 9 qubits) -- everything else is imported evidence
        from a sibling file/topology, not reproduced by a run of this file. *)
     "ValidatedRadius" -> "THIS SCRIPT directly re-verifies only the literal ddt ring at N=3 (wordRing[\"ddt\",1], 9 qubits). The 8- and 11-qubit cases were verified in the STANDALONE ddt_cluster_avn_witness.wl on a DIFFERENT (open PentagonChain) topology, not reachable by this script's own N->reps parametrization -- imported evidence, not reproduced here. NOT established beyond these specific cases on either topology."
  |>,
  "DLA" -> <|
     ...
     "ValidatedRadius" -> "exact ONLY at N=1 pentagon (5 qubits); N=2 pentagons (8 qubits) did not complete in a 150s probe in ddt_cluster_dla.wl -- NOT established beyond N=1"
  |>
|>
