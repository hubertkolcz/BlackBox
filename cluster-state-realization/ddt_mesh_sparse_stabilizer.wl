...
(* ===========================================================================
   ddt_mesh_sparse_stabilizer.wl -- SPARSE, O(n + edges) graph-state stabilizer
   ...
   ddt_cluster_stabilizer.wl (MatrixRank[...,Modulus->2] and
   ...
   files (ddt_cluster_avn_witness.wl, ddt_cluster_dla.wl) -- those are
   fundamentally exponential in qubit count and remain gated at their small-N
   ceilings exactly as before (see ddt_cluster_lie_poisson_bridge.wl Sections
   3-4 and their honesty notes).

   FILES READ IN FULL BEFORE WRITING THIS FILE (per task instructions):
     * black-box-test/ddt_cluster_stabilizer.wl        (the OLD dense method,
       reused verbatim in Section 2 below as the baseline)
     * black-box-test/ddt_cluster_lie_poisson_bridge.wl (confirms the same
     ...
     * mesh-composition/CaseStudies.wl                  (wordRing[word,reps]
       ...
       ddt_cluster_stabilizer.wl -- see the Grep confirmation in the session
   ...
   Run:  wolframscript -file ddt_mesh_sparse_stabilizer.wl
   =========================================================================== *)
...
Do[
  Module[{e1, e2, match},
    e1 = wordRingOrig["ddt", reps]; e2 = fastWordRingEdges["ddt", reps];
...
Do[
  Module[{t1, t2},
    {t1, e1} = AbsoluteTiming[Length[wordRingOrig["ddt", reps]]];
    {t2, e2} = AbsoluteTiming[Length[fastWordRingEdges["ddt", reps]]];
    Print["    reps=", reps, "  qubits=", 9 reps, "   orig: ", t1, "s   fast: ", t2, "s"]],
  {reps, {10, 100, 1000}}];
Print[];

(* ---------------------------------------------------------------------------
   SECTION 2. OLD (DENSE) METHOD -- reused VERBATIM from ddt_cluster_stabilizer.wl
   (identical code also duplicated in ddt_cluster_lie_poisson_bridge.wl
...
section4Results = Table[
  Module[{edges, nQ, tOld, oldR, tNew, newR, agree, tBuild},
    {tBuild, edges} = AbsoluteTiming[fastWordRingEdges["ddt", reps]];
    nQ = 9 reps;
