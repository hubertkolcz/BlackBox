...
(* ===========================================================================
   ddt_mbqc_hawking_certification.wl -- CHSH / contextual-fraction (CF)
   CERTIFICATION STACK for mesh-carved analogue-Hawking pairs, built FROM FIRST
   PRINCIPLES on top of the verified repo stack (ddt_mbqc_sim.wl sparse CHP
   tableau; ddt_mbqc_patterns.wl mesh carving; ddt_mesh_sparse_construction.wl
   the "ddt" pentagon mesh).  No external literature was consulted in building
   ...
   WHAT IS CERTIFIED (all five stages of the task):
     (1) MESH-CARVED HAWKING PAIRS.  A Bell pair is carved from the real "ddt"
   ...
   LOAD-ONLY:  DDTHawkingCertLoadOnly = True; Get[...]  (defs only, no self-check)
   Run:        wolframscript -file ddt_mbqc_hawking_certification.wl   (fast self-check)
   Full stack: ddt_mbqc_hawking_certification_tests.wl  (M=100 @ reps=1000, N>=10^4)
   =========================================================================== *)

If[!ValueQ[DDTHawkingCertDir],
  DDTHawkingCertDir = DirectoryName[$InputFileName]];
Block[{DDTMBQCPatternsLoadOnly = True},
  Get[FileNameJoin[{DDTHawkingCertDir, "ddt_mbqc_patterns.wl"}]]];

(* ---------------------------------------------------------------------------
   SECTION H0.  Pre-registered budget constants (fixed before sampling).
   --------------------------------------------------------------------------- *)
DDTHawkAlpha = 1/100;                 (* family-wise error *)
DDTHawkNTests = 18;                   (* 3 no-disturbance + 15 tomography *)
DDTHawkDelta = N[DDTHawkAlpha/DDTHawkNTests];
DDTHawkEps[nn_]   := Sqrt[Log[2/DDTHawkDelta]/(2 nn)];    (* range-1 (0/1) obs *)
DDTHawkEpsPM[nn_] := 2 Sqrt[Log[2/DDTHawkDelta]/(2 nn)];  (* range-2 (+-1) obs *)

(* ---------------------------------------------------------------------------
   SECTION H1.  Mesh-carved Hawking pairs.

   DDTHawkingSelectPairs[reps, maxM] greedily selects up to maxM disjoint
   ...
   --------------------------------------------------------------------------- *)
DDTHawkingSelectPairs[reps_Integer, maxM_Integer] := Module[
  {edges = DDTMeshEdges[reps], n = DDTMeshN[reps], adj, used = Association[],
   pairs = {}, k = 1, a, b, cA, cB, region},
  adj = DDTAdjacency[edges, n];
...
Options[DDTHawkingBuildCarved] = {"Forced" -> 0};
DDTHawkingBuildCarved[reps_Integer, pairs_List, OptionsPattern[]] := Module[
  {edges = DDTMeshEdges[reps], n = DDTMeshN[reps], forced = OptionValue["Forced"],
   tab, out, frames},
...
(* ---------------------------------------------------------------------------
   ...
   DDTPauliExp[tab, xs, zs] returns the exact expectation <Q> of the Hermitian
   ...
   deterministic-measurement CASE B, generalized to multi-qubit Q):
     ...
     membership test.)
   --------------------------------------------------------------------------- *)
DDTPauliExp[tab_Symbol, xs_List, zs_List] := Module[
  ...
  Scan[Function[i, Module[
     ...
     js = Union[rx, rz];
     rxA = DDTBitLookup[rx]; rzA = DDTBitLookup[rz];
     sxA = DDTBitLookup[sx]; szA = DDTBitLookup[sz];
     s = Mod[2 sr + 2 rr +
         Total[gPhase[Boole[KeyExistsQ[rxA, #]], Boole[KeyExistsQ[rzA, #]],
             Boole[KeyExistsQ[sxA, #]], Boole[KeyExistsQ[szA, #]]] & /@ js], 4];
     If[!(s === 0 || s === 2),
       Print["*** DDTPauliExp PARITY VIOLATION (corrupt tableau) i=", i]; Abort[]];
...
DDTPauliBits["I"] = {False, False}; DDTPauliBits["X"] = {True, False};
DDTPauliBits["Z"] = {False, True};  DDTPauliBits["Y"] = {True, True};

(* exact expectation of Pa (x) Pb on qubits a,b, Pa,Pb in {I,X,Y,Z} *)
DDTPairExp[tab_Symbol, a_Integer, b_Integer, pa_String, pb_String] := Module[
  {ba = DDTPauliBits[pa], bb = DDTPauliBits[pb], xs = {}, zs = {}},
  If[ba[[1]], AppendTo[xs, a]]; If[ba[[2]], AppendTo[zs, a]];
  If[bb[[1]], AppendTo[xs, b]]; If[bb[[2]], AppendTo[zs, b]];
  DDTPauliExp[tab, xs, zs]];

DDTPauliLabels = {"I", "X", "Y", "Z"};

(* 3x3 correlation matrix T[i,j] = <sigma_i^a sigma_j^b>, i,j in {X,Y,Z} order.
   Row index = a-basis, column index = b-basis. *)
DDTPairCorrMatrix[tab_Symbol, a_Integer, b_Integer] :=
  Table[DDTPairExp[tab, a, b, pa, pb], {pa, {"X", "Y", "Z"}}, {pb, {"X", "Y", "Z"}}];

(* exact 4x4 reduced density matrix on {a,b}: (1/4) sum_{Pa,Pb} <Pa Pb> Pa(x)Pb *)
DDTHawkPauliMat["I"] = {{1, 0}, {0, 1}};   DDTHawkPauliMat["X"] = {{0, 1}, {1, 0}};
DDTHawkPauliMat["Y"] = {{0, -I}, {I, 0}};  DDTHawkPauliMat["Z"] = {{1, 0}, {0, -1}};
DDTExactRDM[tab_Symbol, a_Integer, b_Integer] := Module[{acc},
  acc = Sum[
     DDTPairExp[tab, a, b, pa, pb] *
       KroneckerProduct[DDTHawkPauliMat[pa], DDTHawkPauliMat[pb]],
     {pa, DDTPauliLabels}, {pb, DDTPauliLabels}];
  acc/4];

(* the 15 non-identity two-qubit Pauli labels {pa,pb}, pa,pb in {I,X,Y,Z}\{II} *)
DDTHawk15 = DeleteCases[
   Flatten[Table[{pa, pb}, {pa, DDTPauliLabels}, {pb, DDTPauliLabels}], 1],
   {"I", "I"}];
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
DDTHawkingNoDisturbance[reps_Integer, pairs_List, builds_Integer] := Module[
  {hSum = 0, pSum = 0, nShots, bundle},
  nShots = builds * Length[pairs];
  Do[
    bundle = DDTHawkingBuildCarved[reps, pairs, "Forced" -> Automatic];
    Scan[Function[p,
  ...
  Module[{nH = N[hSum/nShots], nP = N[pSum/nShots], epsH, epsP},
   epsH = DDTHawkEps[nShots]; epsP = DDTHawkEps[nShots];
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
DDTHawkingTomography[reps_Integer, pairs_List, buildsPerSetting_Integer] := Module[
  ...
  Do[Module[{pa = st[[1]], pb = st[[2]]},
    ...
    Do[Module[{bundle = DDTHawkingBuildCarved[reps, pairs, "Forced" -> Automatic]},
  ...
  Module[{est = Association[], epsN = DDTHawkEpsPM[nShots]},  (* +-1 correlators *)
...
DDTReconstructRDM[est_Association] := Module[{acc},
  acc = KroneckerProduct[DDTHawkPauliMat["I"], DDTHawkPauliMat["I"]];  (* <II>=1 *)
  acc = acc + Sum[
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
DDTCHSHOptimal[T_] := Module[{sv = SingularValueList[N[T]]},
...
DDTChshSections = {{0, 0}, {0, 1}, {1, 0}, {1, 1}};
DDTChshCtx = Flatten[Table[{x, y}, {x, 0, 1}, {y, 0, 1}], 1];
DDTChshGlob = Tuples[{0, 1}, 4];         (* (a0,a1,b0,b1) *)
DDTChshM = Module[{M = Table[0, {16}, {16}]},
  Do[Module[{x = DDTChshCtx[[c, 1]], y = DDTChshCtx[[c, 2]]},
    Do[Module[{a = DDTChshSections[[s, 1]], b = DDTChshSections[[s, 2]]},
      Do[Module[{t = DDTChshGlob[[g]], ax, by},
        ax = t[[x + 1]]; by = t[[2 + y + 1]];
        If[{ax, by} == {a, b}, M[[4 (c - 1) + s, g]] = 1]], {g, 16}]], {s, 4}]], {c, 4}];
  M];
DDTChshIsoTable[s_] := Module[{c = s/4, e = Table[0, {16}]},
  Do[Module[{x = DDTChshCtx[[k, 1]], y = DDTChshCtx[[k, 2]], corr},
    corr = If[{x, y} == {1, 1}, -c, c];
    e[[4 (k - 1) + 1 ;; 4 (k - 1) + 4]] =
       {(1 + corr)/4, (1 - corr)/4, (1 - corr)/4, (1 + corr)/4}], {k, 4}];
  e];
DDTCFofSLP[s_] := Module[{e = N[DDTChshIsoTable[s]], sol},
  sol = LinearProgramming[-ConstantArray[1., 16], -DDTChshM, -e,
...
(* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- *)
DDTHawkRandPauli[tab_Symbol, q_Integer] := Module[{r = RandomInteger[{0, 3}]},
...
DDTHawkNoisyCorr[reps_Integer, pair_, p_, K_Integer] := Module[
  {acc = Table[0., {3}, {3}], a = pair["partner"], b = pair["hawking"]},
  Do[Module[{bundle = DDTHawkingBuildCarved[reps, {pair}, "Forced" -> 0], tab},
     tab = bundle["tab"];
     If[RandomReal[] < p,
       DDTHawkRandPauli[tab, a]; DDTHawkRandPauli[tab, b]];
     acc += N[DDTPairCorrMatrix[tab, a, b]];
     FreeTableau[tab]], {K}];
  acc/K];
...
DDTHawkNoiseCurve[reps_Integer, pair_, ps_List, K_Integer] :=
  Table[Module[{T = DDTHawkNoisyCorr[reps, pair, p, K], s},
     s = DDTCHSHExplicit[T];
     <|"p" -> p, "CHSH" -> s, "CF" -> DDTCFofS[s],
       "CHSHexact" -> (1 - p) 2 Sqrt[2.], "CFexact" -> DDTCFofS[(1 - p) 2 Sqrt[2.]]|>],
   {p, ps}];

(* exact depolarizing crossings from CHSH(p)=(1-p) 2 sqrt2 *)
DDTHawkPForCHSH[target_] := 1 - target/(2 Sqrt[2]);   (* p where CHSH(p)=target *)

(* ---------------------------------------------------------------------------
   SECTION H9.  Guarded self-check (fast).  Full-scale stack: the tests file.
   --------------------------------------------------------------------------- *)
If[!TrueQ[DDTHawkingCertLoadOnly],
  Module[{reps, pairs, bundle, tab, a, b, rdmExact, rdmSV, svOK, T, sOpt, sExp,
    ...
    Print["=== ddt_mbqc_hawking_certification.wl self-check ==="];
    Print["    pre-registered budget: alpha=", N[DDTHawkAlpha], " over ",
      DDTHawkNTests, " tests -> per-test delta=", DDTHawkDelta];
    ...
    reps = 1; pairs = DDTHawkingSelectPairs[reps, 1];
    a = pairs[[1]]["partner"]; b = pairs[[1]]["hawking"];
    bundle = DDTHawkingBuildCarved[reps, pairs, "Forced" -> 0]; tab = bundle["tab"];
    rdmExact = DDTExactRDM[tab, a, b];
    Module[{v = StateVectorFromTableau[tab], nn = DDTMeshN[reps], T2, other},
    ...
    cfTs = DDTCFofS[2 Sqrt[2]]; cfLocal = DDTCFofS[2]; cf225 = DDTCFofS[2.25];
    cfLPTs = DDTCFofSLP[2 Sqrt[2]]; cfLPLocal = DDTCFofSLP[2]; cfLP225 = DDTCFofSLP[2.25];
    ...
    reps = 30; pairs = DDTHawkingSelectPairs[reps, 20];
    SeedRandom[20260713];
    nd = DDTHawkingNoDisturbance[reps, pairs, 30];   (* 20*30=600 shots *)
    ...
    bundle = DDTHawkingBuildCarved[reps, pairs, "Forced" -> 0];
    rdmExact = DDTExactRDM[bundle["tab"], pairs[[1]]["partner"], pairs[[1]]["hawking"]];
    FreeTableau[bundle["tab"]];
    SeedRandom[20260714];
    tomo = DDTHawkingTomography[reps, pairs, 20];    (* 20*20=400 shots/setting *)
    recRDM = DDTReconstructRDM[tomo["est"]];
    tomoDev = Max[Table[Abs[Lookup[tomo["est"], Key[pl], 0] -
        Re[Tr[ConjugateTranspose[KroneckerProduct[DDTHawkPauliMat[pl[[1]]], DDTHawkPauliMat[pl[[2]]]]] . rdmExact]]],
       {pl, DDTHawk15}]];
    ...
    reps = 3; pairs = DDTHawkingSelectPairs[reps, 1];
    SeedRandom[20260715];
    noise = DDTHawkNoiseCurve[reps, pairs[[1]], {0, 0.1, 0.2, 0.25, 0.3}, 400];
    ...
    reps = 100; headPairs = DDTHawkingSelectPairs[reps, 100];
    bundle = DDTHawkingBuildCarved[reps, headPairs, "Forced" -> 0]; tab = bundle["tab"];
    headOK = AllTrue[headPairs, Function[p,
       DDTCHSHOptimal[DDTPairCorrMatrix[tab, p["partner"], p["hawking"]]] > 2.8]];
    FreeTableau[tab];
    Print["    M=", Length[headPairs], " pairs @ reps=", reps,
      "; every pair CHSH>2.8 (== 2 sqrt2)? ", headOK];

    Print[];
    Print["DDTHawkingCertSelfCheck: ", <|
