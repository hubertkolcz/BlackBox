...
(* ===========================================================================
   ddt_mbqc_sim_tests.wl -- FULL validation suite for ddt_mbqc_sim.wl, the
   ...
   MemoryInUse appear only in benchmark prints):
     V0. Micro-foundations (delegated to DDTMBQCRunV0Foundations[] in the
     ...
     V1. Graph-state init cross-check on 6 graphs (C5 ring, path P4, star
         K_{1,3}, complete K4, edgeless n=3, pentagon mesh "ddt" reps=1)
   ...
   Run: wolframscript -file C:/Users/cp/Desktop/black-box/cluster-state-realization/ddt_mbqc_sim_tests.wl
   =========================================================================== *)
...
Module[{libPath},
  libPath = If[StringQ[$InputFileName] && $InputFileName =!= "",
    FileNameJoin[{DirectoryName[$InputFileName], "ddt_mbqc_sim.wl"}],
    "C:/Users/cp/Desktop/black-box/cluster-state-realization/ddt_mbqc_sim.wl"];
  Print["Loading library: ", libPath];
  Get[libPath]];

Print["=== V0: micro-foundations (WL idioms + exact g/gate algebra vs dense conjugation) ==="];
v0Result = DDTMBQCRunV0Foundations[];
...
refProb[v_, P_, b_] := Module[{vd = Conjugate[v], nn, ee, p},
  ...
  p = (nn + (-1)^b ee)/(2 nn);
  DDTAssert[MemberQ[{0, 1/2, 1}, p], "refProb: not in {0,1/2,1}", {p, b}];
  p];

(* inline copy of wordRingEdgesFast from
   cluster-state-realization/ddt_mesh_sparse_construction.wl (that file is a read-only
...
v1cases = {
  ...
  {"Edgeless3",   3, {}},
  {"MeshDDTreps1", 9, wordRingEdgesFastLocal["ddt", 1]}};
...
CIRCUITSPERN = If[ValueQ[DDTMBQCCircuitsPerNOverride], DDTMBQCCircuitsPerNOverride,
...
runV3Tier[reps_, validate_] := Module[
  {L = 3 reps, nQ = 9 reps, tBuild, edges, tTab, tab, m, targets, recs = {},
   times = {}, stats},
  {tBuild, edges} = AbsoluteTiming[wordRingEdgesFastLocal["ddt", reps]];
  ...
  Do[Module[{t, rec},
     ...
     DDTAssert[wellFormedRecQ[rec], "V3: malformed measurement record", rec];
...
{v3RepsA, v3RepsB} = If[ValueQ[DDTMBQCV3TiersOverride], DDTMBQCV3TiersOverride,
...
runV3bTier[reps_] := Module[
  {L = 3 reps, nQ = 9 reps, edges, tab, tZ, tX, nZ = 2000, nX = 2000, zq, xq,
   zrecs, xrecs, stats},
  edges = wordRingEdgesFastLocal["ddt", reps];
  ...
  DDTAssert[Max[Join[zq, xq]] <= nQ, "V3b: target qubits out of range", {reps, Max[xq]}];
  SeedRandom[43];
...
{v3bRepsA, v3bRepsB} = If[ValueQ[DDTMBQCV3bTiersOverride], DDTMBQCV3bTiersOverride,
...
|>;
Print["=== V4: SUMMARY ==="];
Print["DDTMBQCSimulatorSummary: ", DDTMBQCSimulatorSummary];
If[!TrueQ[DDTMBQCSimulatorSummary["AllPass"]], Print["*** SUITE FAILED ***"]; Exit[1]];
