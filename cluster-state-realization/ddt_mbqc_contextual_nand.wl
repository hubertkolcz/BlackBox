...
(* ===========================================================================
   ...
   independent of the sparse tableau simulator):
     ...
     STEP 1  CARVE. On the ACTUAL pentagon-mesh graph state
             |G> = prod_{(i,j) in wordRingEdgesFast["ddt",reps]} CZ_ij |+>^n
   ...
   Run:  wolframscript -file ddt_mbqc_contextual_nand.wl
   =========================================================================== *)

(* ---------------------------------------------------------------------------
   wordRingEdgesFast: copied VERBATIM from
   cluster-state-realization/ddt_mesh_sparse_construction.wl (O(L) replacement for the
...
verifyCarve[reps_] := Module[
   ...
   edges = wordRingEdgesFast["ddt", reps];
   nM = n - 3;
   ...
   Print["  reps=", reps, ": n=", n, " qubits, ", Length[edges],
     " edges from wordRingEdgesFast[\"ddt\",", reps, "] (edge list NOT edited)"];
...
nQ = 9; edges1 = wordRingEdgesFast["ddt", 1];
