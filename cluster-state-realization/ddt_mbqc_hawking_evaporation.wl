...
(* ===========================================================================
   ddt_mbqc_hawking_evaporation.wl -- black-hole information-dynamics module
   built FROM FIRST PRINCIPLES on top of the verified sparse-CHP stabilizer
   simulator (ddt_mbqc_sim.wl) and pattern layer (ddt_mbqc_patterns.wl).
   ...
   API SURFACE:
     Part 1 (stabilizer entropy):
       DDTStabEntropy[tab, A]              exact S(A) in bits, GF(2) rank
       DDTStabPurity[tab, A]               exact 2^-S(A)
       DDTvnEntropyFromState[tab, A]       validation-only von Neumann via
                                           StateVectorFromTableau (n<=10)
     Part 2 (Page curve):
       DDTRandom2QClifford[tab, a, b]      circuit-model random 2-qubit Clifford
       DDTScramble[tab, qs, sweeps]        circuit-model scrambler on a qubit set
       DDTPageCurveDirect[n, opts]         single-realization Page curve (model b)
       DDTPageCurveMesh[reps, opts]        mesh-cluster-state Page curve (model a)
       DDTMeshBellPair[reps, edge]         carve+validate a mesh Bell pair (S=1)
       DDTLubkinPurity[dA, dB]             exact (d_A+d_B)/(d_A d_B+1)
       DDTPageRenyi2Closed[n, r]           exact closed-form Renyi-2 S2(N_rad=r)
       DDTPageRenyi2Ensemble[n, shots]     sampled ensemble Renyi-2 curve
     Part 3 (Hayden-Preskill):
       DDTFindMaximalScrambler[m, ...]     search a maximal Clifford scrambler
       DDTApplyScrambler / DDTApplyScramblerConj
       DDTHPDecodeEPR / DDTHPDecodeDeterministic / DDTHPTeleportFidelity

   LOAD-ONLY USE:  DDTHawkingLoadOnly = True; Get[".../ddt_mbqc_hawking_evaporation.wl"]
   loads definitions with no side effects.  Running the file directly executes
   a fast self-check.  The FULL validation suite is
   ddt_mbqc_hawking_evaporation_tests.wl.
   =========================================================================== *)

If[!ValueQ[DDTHawkingDir], DDTHawkingDir = DirectoryName[$InputFileName]];
Block[{DDTMBQCPatternsLoadOnly = True, DDTMBQCLoadOnly = True},
  Get[FileNameJoin[{DDTHawkingDir, "ddt_mbqc_patterns.wl"}]]];
...
DDTStabRestrict[tab_Symbol, A_List] := Module[{n = tab["n"], pos},
...
DDTStabEntropy[tab_Symbol, A_List] :=
  If[A === {}, 0, MatrixRank[DDTStabRestrict[tab, A], Modulus -> 2] - Length[A]];

DDTStabPurity[tab_Symbol, A_List] := 2^(-DDTStabEntropy[tab, A]);
...
DDTReducedRhoState[tab_Symbol, A_List] := Module[
...
DDTvnEntropyFromState[tab_Symbol, A_List] := Log2[MatrixRank[DDTReducedRhoState[tab, A]]];
...
(* exact flatness + entropy: returns <|"S"->integer, "Flat"->bool|>.
   ...
   exact.  S(A) = log2(rk). *)
DDTStateEntropyReport[tab_Symbol, A_List] := Module[{rho, rhoN, rk},
  rho = DDTReducedRhoState[tab, A];
...
(* ---------------------------------------------------------------------------
   ...
   CIRCUIT-MODEL DECLARATION: DDTRandom2QClifford and DDTScramble apply
...
(* a depth-8 random Clifford WORD on {a,b}: 8 random generators from
   ...
   element; it is a cheap mixing block that, composed over many DDTScramble
   sweeps, drives the whole register to a Clifford 2-design (verified against
   the Lubkin closed form in DDTPageRenyi2Ensemble).  Depth 8 trades per-block
   uniformity for speed -- essential for the n=200 scale point. *)
DDTCliffordWordLen = 8;
DDTRandom2QClifford[tab_Symbol, a_Integer, b_Integer] := (
...
DDTScramble[tab_Symbol, qs_List, sweeps_Integer] := (
  Do[Module[{perm = RandomSample[qs]},
     Do[DDTRandom2QClifford[tab, perm[[2 j - 1]], perm[[2 j]]],
...
Options[DDTPageCurveDirect] = {"Sweeps" -> 6, "Seed" -> Automatic};
DDTPageCurveDirect[n_Integer?Positive, OptionsPattern[]] := Module[
  ...
  {t, curve} = AbsoluteTiming[Module[{cv},
    ...
    bh = Range[n];
    cv = {DDTStabEntropy[tab, rad]};                (* S({}) = 0 *)
    Do[
      DDTScramble[tab, bh, sweeps];
      rad = Append[rad, First[bh]]; bh = Rest[bh];  (* emit one qubit *)
      AppendTo[cv, DDTStabEntropy[tab, rad]],
      {n}];
    cv]];
...
(* MODEL (a): MESH-CLUSTER Page curve.  The initial black-hole microstate IS
   ...
   wordRingEdgesFast["ddt",reps] edge list -- a genuinely entangled stabilizer
...
Options[DDTPageCurveMesh] = {"Sweeps" -> 2, "Seed" -> Automatic};
DDTPageCurveMesh[reps_Integer?Positive, OptionsPattern[]] := Module[
  {sweeps = OptionValue["Sweeps"], seed = OptionValue["Seed"],
   n = DDTMeshN[reps], edges = DDTMeshEdges[reps], tab, bh, rad = {}, curve, t},
  If[seed =!= Automatic, SeedRandom[seed]];
  {t, curve} = AbsoluteTiming[Module[{cv},
    tab = NewGraphStateTableau[n, edges];
    bh = Range[n];
    cv = {DDTStabEntropy[tab, rad]};
    Do[DDTScramble[tab, bh, sweeps];
       rad = Append[rad, First[bh]]; bh = Rest[bh];
       AppendTo[cv, DDTStabEntropy[tab, rad]], {n}];
    cv]];
...
(* SCALE-DEMO Page curve (single realization, large n).  CIRCUIT-MODEL /
   ...
   (circuit-model) scrambling differs from DDTPageCurveDirect. *)
Options[DDTPageCurveScrambledState] = {"Sweeps" -> 14, "Seed" -> Automatic};
DDTPageCurveScrambledState[n_Integer?Positive, OptionsPattern[]] := Module[
  {sweeps = OptionValue["Sweeps"], seed = OptionValue["Seed"], tab, curve, t},
  If[seed =!= Automatic, SeedRandom[seed]];
  {t, curve} = AbsoluteTiming[
    tab = NewGraphStateTableau[n, {}];
    DDTScramble[tab, Range[n], sweeps];
    Table[DDTStabEntropy[tab, Range[r]], {r, 0, n}]];
  <|"n" -> n, "curve" -> curve, "pageTime" -> Ceiling[n/2],
    "peak" -> Max[curve], "final" -> Last[curve],
...
DDTMeshBellPair[reps_Integer, edge_List] := Module[
  {n = DDTMeshN[reps], edges = DDTMeshEdges[reps], tab, others, s},
  DDTAssert[MemberQ[edges, Sort[edge]], "DDTMeshBellPair: edge not in mesh", edge];
  tab = NewGraphStateTableau[n, edges];
  others = Complement[Range[n], edge];
  Scan[MeasurePauli[tab, #, "Z"] &, others];
  s = DDTStabEntropy[tab, {First[edge]}];
  FreeTableau[tab];
...
DDTLubkinPurity[dA_Integer, dB_Integer] := (dA + dB)/(dA dB + 1);
...
DDTPageRenyi2Closed[n_Integer, r_Integer] :=
  -Log2[DDTLubkinPurity[2^r, 2^(n - r)]];
...
(* SAMPLED ensemble Renyi-2 curve.  For each of `shots` realizations: build a
   ...
   comparison. *)
Options[DDTPageRenyi2Ensemble] = {"Sweeps" -> 16, "Seed" -> Automatic};
DDTPageRenyi2Ensemble[n_Integer, shots_Integer, OptionsPattern[]] := Module[
  {sweeps = OptionValue["Sweeps"], seed = OptionValue["Seed"],
   acc, t, means, s2},
  If[seed =!= Automatic, SeedRandom[seed]];
  {t, acc} = AbsoluteTiming[Module[{sum = ConstantArray[0, n + 1], tab},
    Do[tab = NewGraphStateTableau[n, {}];
       DDTScramble[tab, Range[n], sweeps];
       sum = sum + Table[DDTStabPurity[tab, Range[r]], {r, 0, n}];
  ...
  <|"n" -> n, "shots" -> shots, "meanPurity" -> means, "S2sampled" -> s2,
    "S2closed" -> Table[N[DDTPageRenyi2Closed[n, r]], {r, 0, n}],
    "time" -> t|>];
...
DDTHPidmap = Association[Table[i -> i, {i, 1, 12}]];    (* U side: position=qubit *)
DDTHPdecmap = Association[1 -> 9, 2 -> 6, 3 -> 7, 4 -> 8]; (* U* side remap *)
...
DDTHPGateApply[tab_Symbol, g_, qmap_, conj_, p_] := Module[{qs},
  Switch[g[[1]],
...
DDTHPRowWeight[tab_Symbol, h_Integer] := Length[Union[tab["rowX", h], tab["rowZ", h]]];
...
(* CONSTRUCT a MAXIMAL Clifford scrambler on m qubits: search random depth-4m
   ...
   (which must be >= 3). *)
Options[DDTFindMaximalScrambler] = {"Seed" -> 123, "MaxTries" -> 4000};
DDTHPYImageWeight[tab_Symbol, i_Integer, m_Integer] := Length[Union[
  SymDiff[tab["rowX", i], tab["rowX", m + i]],
  SymDiff[tab["rowZ", i], tab["rowZ", m + i]]]];
DDTFindMaximalScrambler[m_Integer, OptionsPattern[]] := Module[
  ...
  While[True, tries++;
    ...
    Do[DDTHPGateApply[tab, g, DDTHPidmap, False, 0], {g, gates}];
    weights = Flatten[Table[{DDTHPRowWeight[tab, i], DDTHPRowWeight[tab, m + i],
       DDTHPYImageWeight[tab, i, m]}, {i, m}]];
    minw = Min[weights];
    FreeTableau[tab];
...
DDTHPxx = KroneckerProduct[{{0, 1}, {1, 0}}, {{0, 1}, {1, 0}}];
DDTHPzz = KroneckerProduct[{{1, 0}, {0, -1}}, {{1, 0}, {0, -1}}];
DDTHPyy = KroneckerProduct[{{0, -I}, {I, 0}}, {{0, -I}, {I, 0}}];
...
Options[DDTHPDecodeEPR] = {"NoiseP" -> 0};
DDTHPDecodeEPR[gates_List, OptionsPattern[]] := Module[
  {p = OptionValue["NoiseP"], tab, pr, rho, tr, XX, ZZ, YY},
  tab = DDTHPBuildInstance[];
  Do[DDTHPGateApply[tab, g, DDTHPidmap, False, p], {g, gates}];
  Do[DDTHPGateApply[tab, g, DDTHPdecmap, True, p], {g, gates}];
  ...
  rho = DDTReducedRhoState[tab, {5, 10}]; tr = Tr[rho];
  FreeTableau[tab];
  If[tr === 0, Return[<|"successProb" -> pr, "Fe" -> Missing["ProjFail"]|>]];
  rho = rho/tr;
  XX = Re[Tr[rho . DDTHPxx]]; ZZ = Re[Tr[rho . DDTHPzz]]; YY = Re[Tr[rho . DDTHPyy]];
...
DDTHPDecodeDeterministic[gates_List] := Module[{tab, sJoint, sHalf},
  tab = DDTHPBuildInstance[];
  Do[DDTHPGateApply[tab, g, DDTHPidmap, False, 0], {g, gates}];
  Do[DDTHPGateApply[tab, g, DDTHPdecmap, True, 0], {g, gates}];
  ...
  sJoint = DDTStabEntropy[tab, {5, 10}]; sHalf = DDTStabEntropy[tab, {5}];
  FreeTableau[tab];
...
DDTHPketPrep[tab_Symbol, q_Integer, s_String] := Switch[s,
  "0", ApplyH[tab, q],                                  (* |+> -> H -> |0> *)
  ...
  "+i", ApplyS[tab, q],                                 (* |+i> *)
  "-i", ApplySdg[tab, q]];                              (* |-i> *)
DDTHPinputBasis = <|"0" -> "Z", "1" -> "Z", "+" -> "X", "-" -> "X", "+i" -> "Y", "-i" -> "Y"|>;
...
DDTHPTeleportInput[gates_List, s_String] := Module[{tab, det},
  tab = NewGraphStateTableau[10, {{2, 6}, {3, 7}, {4, 8}, {9, 10}}];
  Scan[ApplyH[tab, #] &, {6, 7, 8, 10}];               (* B-B', P-R Bell pairs *)
  DDTHPketPrep[tab, 1, s];                              (* diary on A=1 *)
  Do[DDTHPGateApply[tab, g, DDTHPidmap, False, 0], {g, gates}];
  Do[DDTHPGateApply[tab, g, DDTHPdecmap, True, 0], {g, gates}];
  ApplyCNOT[tab, 3, 7]; ApplyH[tab, 3]; MeasurePauli[tab, 3, "Z"]; MeasurePauli[tab, 7, "Z"];
  ApplyCNOT[tab, 4, 8]; ApplyH[tab, 4]; MeasurePauli[tab, 4, "Z"]; MeasurePauli[tab, 8, "Z"];
  det = MeasurePauli[tab, 10, DDTHPinputBasis[s]]["Deterministic"];
  FreeTableau[tab]; det];
...
DDTHPTeleportFidelity[gates_List] := Module[{res},
  res = DDTHPTeleportInput[gates, #] & /@ {"0", "1", "+", "-", "+i", "-i"};
...
DDTHPNoiseSweep[gates_List, ps_List, shots_Integer, seed_Integer] := Module[{},
  SeedRandom[seed];
  Table[Module[{sum = 0, cnt = 0, fail = 0, f},
    Do[f = DDTHPDecodeEPR[gates, "NoiseP" -> p]["Fe"];
       If[MissingQ[f], fail++, sum += f; cnt++], {shots}];
    <|"p" -> p, "meanFe" -> N[sum/Max[cnt, 1]], "valid" -> cnt, "projFail" -> fail|>],
   {p, ps}]];

(* ---------------------------------------------------------------------------
   SELF-CHECK (skipped when DDTHawkingLoadOnly = True).
   --------------------------------------------------------------------------- *)
If[!TrueQ[DDTHawkingLoadOnly],
  Module[{v0, okE, tab, bp, pc, r2},
    Print["=== ddt_mbqc_hawking_evaporation.wl self-check ==="];
    ...
    okE = And @@ Table[Module[{nn = RandomInteger[{2, 8}], t2, A, sG, sV},
       ...
       DDTScramble[t2, Range[nn], 4];
       A = Sort[RandomSample[Range[nn], RandomInteger[{0, nn}]]];
       sG = DDTStabEntropy[t2, A];
       sV = DDTStateEntropyReport[t2, A];
       FreeTableau[t2];
       (sG === sV["S"]) && sV["Flat"]], {60}];
    ...
    r2 = DDTPageRenyi2Ensemble[8, 150, "Seed" -> 99];
    ...
    Module[{sc, epr, det, tele, idepr, sweep},
      ...
      idepr = DDTHPDecodeEPR[{}];
      Print["    identity control: Fe=", idepr["Fe"], " -> avg teleport fidelity=",
        (2 idepr["Fe"] + 1)/3];
      sweep = DDTHPNoiseSweep[sc["gates"], {0.01, 0.02}, 150, 5];
