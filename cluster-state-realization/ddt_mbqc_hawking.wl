...
(* ===========================================================================
   ddt_mbqc_hawking.wl -- MASTER DELIVERABLE / consolidated runner for the
   ...
   Loads the two build modules (definitions only, no side effects)
     * ddt_mbqc_hawking_evaporation.wl    Page curve, Renyi-2 ensemble,
                                          Hayden-Preskill decoding
     * ddt_mbqc_hawking_certification.wl  mesh-carved Hawking pairs, CHSH /
                                          contextual-fraction certification
   and executes a representative scoreboard with a consolidated PASS/FAIL
   verdict, following the ddt_mbqc_master.wl conventions.
   ...
   from ddt_mbqc_hawking_evaporation.wl --
   ...
   from ddt_mbqc_hawking_certification.wl --
   ...
   injection outcome (ddt_mbqc_patterns.wl header); BV/Grover/NAND force
   ...
   file permitted to read that sheet is ddt_mbqc_hawking_confirmation.wl,
   ...
   (ddt_mbqc_hawking_review_probe.wl, reviewer-owned dense re-simulations)
   ...
   Run:  wolframscript -file ddt_mbqc_hawking.wl        (~2-4 minutes)
   Full suites: ddt_mbqc_hawking_evaporation_tests.wl,
     ddt_mbqc_hawking_certification_tests.wl, ddt_mbqc_hawking_confirmation.wl,
     ddt_mbqc_hawking_review_probe.wl (same directory).
   No cloud calls of any kind, ever, by design.
   =========================================================================== *)

If[!ValueQ[DDTHawkMasterDir], DDTHawkMasterDir = DirectoryName[$InputFileName]];
Block[{DDTHawkingLoadOnly = True, DDTMBQCPatternsLoadOnly = True,
       DDTMBQCLoadOnly = True},
  Get[FileNameJoin[{DDTHawkMasterDir, "ddt_mbqc_hawking_evaporation.wl"}]]];
Block[{DDTHawkingCertLoadOnly = True, DDTMBQCPatternsLoadOnly = True},
  Get[FileNameJoin[{DDTHawkMasterDir, "ddt_mbqc_hawking_certification.wl"}]]];
...
Print["############################################################"];
Print["ddt_mbqc_hawking.wl -- Hawking suite consolidated scoreboard"];
Print["############################################################"];
Print[];
...
Module[{t, ok},
  ...
  {t, ok} = AbsoluteTiming[And @@ Table[
    Module[{nn = RandomInteger[{2, 9}], tb, A, sG, rep},
      ...
      sG = DDTStabEntropy[tb, A];
      rep = If[A === {}, <|"S" -> 0, "Flat" -> True|>, DDTStateEntropyReport[tb, A]];
      FreeTableau[tb];
...
Module[{t, r2, maxd, okA, pc, okB, pm, okC},
  {t, r2} = AbsoluteTiming[DDTPageRenyi2Ensemble[12, 60, "Seed" -> 112]];
  ...
  {t, pc} = AbsoluteTiming[DDTPageCurveScrambledState[100, "Seed" -> 100, "Sweeps" -> 14]];
  ...
  {t, pm} = AbsoluteTiming[DDTPageCurveMesh[2, "Seed" -> 7, "Sweeps" -> 4]];
...
Module[{t, sc, epr, det, tele, idc, sweep, okS, okE, okD, okT, okI, okN, tt},
  {t, sc} = AbsoluteTiming[DDTFindMaximalScrambler[4, "Seed" -> 123]];
  okS = sc["isMaximal"];
  ...
  {tt, epr} = AbsoluteTiming[DDTHPDecodeEPR[sc["gates"]]];
  ...
  {tt, sweep} = AbsoluteTiming[DDTHPNoiseSweep[sc["gates"], {0.01, 0.02}, 120, 5]];
...
Module[{t, pairs, bundle, tab, T, okT, sOpt, okS, cfTs, cfLPTs, cf0, cfLP0,
  ...
  {t, T} = AbsoluteTiming[
    pairs = DDTHawkingSelectPairs[3, 1];
    bundle = DDTHawkingBuildCarved[3, pairs, "Forced" -> 0];
    tab = bundle["tab"];
    With[{m = DDTPairCorrMatrix[tab, pairs[[1]]["partner"], pairs[[1]]["hawking"]]},
      FreeTableau[tab]; m]];
  ...
  {tLP, {cfTs, cfLPTs, cf0, cfLP0, cf225, cfLP225}} = AbsoluteTiming[
    {DDTCFofS[2 Sqrt[2]], DDTCFofSLP[2 Sqrt[2]], DDTCFofS[2], DDTCFofSLP[2],
     DDTCFofS[2.25], DDTCFofSLP[2.25]}];
...
(* ---------------------------------------------------------------------------
   ...
   confrontation; full sampled version = ddt_mbqc_hawking_confirmation.wl).
   --------------------------------------------------------------------------- *)
...
Module[{t1ok, peaks, tfd, xx, yy, zz, sA, pur, t3ok, tab, verdicts, t},
  {t, peaks} = AbsoluteTiming[
    Table[{n, N[Max[Table[DDTPageRenyi2Closed[n, r], {r, 0, n}]]]*ln2}, {n, {8, 10, 12}}]];
  ...
  xx = DDTPairExp[tab, 1, 2, "X", "X"]; yy = DDTPairExp[tab, 1, 2, "Y", "Y"];
  zz = DDTPairExp[tab, 1, 2, "Z", "Z"];
  sA = DDTStabEntropy[tab, {1, 3, 5}]; pur = DDTStabPurity[tab, {1, 2}];
  FreeTableau[tab];
