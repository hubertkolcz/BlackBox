...
(* ===========================================================================
   ddt_cluster_stabilizer.wl -- EFFICIENT (GF(2) binary symplectic) stabilizer
   tableau for a pentagon-mesh CZ cluster/graph state, parametrized by a
   direct/twisted gluing word (word="ddt" repeated, or any other word).
   ...
   Run:  wolframscript -file ddt_cluster_stabilizer.wl
   =========================================================================== *)
...
(* ---------------------------------------------------------------------------
   ...
   edge-to-edge, the gluing orientation (direct "d" / twisted "t") at each junction
...
(* helper: exact word of length nb following the repeating "ddt" pattern,
   ...
   wordRing["ddt",reps] directly, i.e. exact multiples of 3, per the task's
   own suggested convention). *)
ddtWord[nb_Integer] := StringTake[StringRepeat["ddt", Ceiling[nb/3] + 1], nb];
...
(* ---------------------------------------------------------------------------
   ...
   pentagons glued via the word "ddt" repeated (wordRing["ddt",reps]; actual
...
runScalingTests[] := Module[{requested = {2, 5, 20, 100}, results},
   results = Table[
     ...
     {nb, requested}];
   Print["=== SECTION 4: scaling of the GF(2) tableau method, word=\"ddt\" repeated ==="];
...
(* ===========================================================================
   RUN
   =========================================================================== *)
Print["=== EFFICIENT (GF(2)) STABILIZER TABLEAU FOR DDT-GLUED PENTAGON CLUSTER STATES ==="];
Print[];
