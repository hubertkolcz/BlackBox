...
(* ===========================================================================
   ddt_mbqc_master.wl -- MASTER RUNNER for the pentagon-mesh MBQC suite.
   ...
   KNOWN LIMITATIONS carried over from the libraries (documented there):
     * TeleportWire post-selects the input-injection outcome (Gadget 1 only;
       see ddt_mbqc_patterns.wl header) -- BV/Grover/NAND force no outcomes.
     * The Grover cluster pattern is calibrated to the k == 0 (mod 3)
       pentagon context of the period-3 "ddt" word.

   CLOUD-DELEGABLE STRUCTURE (no cloud calls HERE, ever, by design): the Get
   chain is fully relative -- this file loads ddt_mbqc_patterns.wl (which
   loads ddt_mbqc_sim.wl) from ITS OWN directory via $InputFileName, with no
   ...
   Run:  wolframscript -file ddt_mbqc_master.wl        (~1-2 minutes)
   Full suites: ddt_mbqc_sim_tests.wl, ddt_mbqc_patterns_tests.wl,
     ddt_mbqc_contextual_nand.wl, ddt_mbqc_scale_run.wl (same directory).
   =========================================================================== *)

(* --- load chain (relative; inline libraries below this marker for cloud) --- *)
If[!ValueQ[DDTMasterDir], DDTMasterDir = DirectoryName[$InputFileName]];
Block[{DDTMBQCPatternsLoadOnly = True, DDTMBQCLoadOnly = True},
  Get[FileNameJoin[{DDTMasterDir, "ddt_mbqc_patterns.wl"}]]];
...
Print["############################################################"];
Print["ddt_mbqc_master.wl -- consolidated medium-size validation"];
Print["############################################################"];
Print[];
...
Module[{t, v0},
  {t, v0} = AbsoluteTiming[DDTMBQCRunV0Foundations[]];
...
(* ---------------------------------------------------------------------------
   ...
   sparse tableau.  Settings/signs from ddt_mbqc_contextual_nand.wl (proved
   there exhaustively in exact arithmetic on the actual mesh) and re-validated
   against the tableau in ddt_mbqc_scale_run.wl V4-V6:
...
DDTMasterORGadget[tab_Symbol, {s1_, s2_, s3_}, c_List, a_Integer, b_Integer] :=
...
Module[{t, ok},
  ...
  {t, ok} = AbsoluteTiming[And @@ Flatten[Table[
     Module[{tab, adj, zarr, c, out},
       ...
       out = DDTMasterORGadget[tab, {1, 2, 3}, c, a, b];
       FreeTableau[tab];
       out === Max[a, b]],
...
Module[{nAdd = 5, reps, n, edges, tab, adj, triples, structOK, survivors,
  ...
  nandGate[x_, y_] := Module[{tr, out},
    ...
    out = DDTMasterORGadget[tab, tr, cAll[[nextTriple]], 1 - x, 1 - y];
