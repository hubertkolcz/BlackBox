...
(* ===========================================================================
   ddt_mbqc_hawking_evaporation_tests.wl -- FULL validation suite / test runner
   for ddt_mbqc_hawking_evaporation.wl.  Every test prints [PASS]/[FAIL] and a
   ...
   Run:  wolframscript -file ddt_mbqc_hawking_evaporation_tests.wl
   (Includes the n=200 scale point -- expect ~8-9 minutes.)
   =========================================================================== *)

If[!ValueQ[DDTTestDir], DDTTestDir = DirectoryName[$InputFileName]];
Block[{DDTHawkingLoadOnly = True},
  Get[FileNameJoin[{DDTTestDir, "ddt_mbqc_hawking_evaporation.wl"}]]];
...
Print["############################################################"];
Print["ddt_mbqc_hawking_evaporation_tests.wl -- full validation"];
Print["############################################################"];
...
Module[{t, ok},
  (* independent formula: for a GRAPH state, S(A) = GF(2) cut-rank of the
     A x B biadjacency submatrix (Hein et al.).  Cross-check vs DDTStabEntropy
  ...
  {t, ok} = AbsoluteTiming[And @@ Table[
     Module[{n = RandomInteger[{4, 9}], edges, adj, tab, A, B, sG, sCut},
       ...
       FreeTableau[tab];
       sG === sCut], {80}]];
  check["80 random GRAPH states: DDTStabEntropy === GF(2) biadjacency cut-rank (independent formula)", ok];
  addRow["T1 graph cut-rank", "80 cases", t, ok]];

Module[{bell, ghz, tab},
  bell = DDTMeshBellPair[1, {2, 3}]["entropy"];
  ...
  ghz = {DDTStabEntropy[tab, {2}], DDTStabEntropy[tab, {1, 3}], DDTStabEntropy[tab, {1, 2, 3}]};
  FreeTableau[tab];
...
Module[{pc, mono, ok},
  pc = DDTPageCurveDirect[16, "Seed" -> 4242];
...
Module[{pc, ok},
  pc = DDTPageCurveMesh[2, "Seed" -> 7, "Sweeps" -> 4];   (* n=18 mesh qubits *)
...
Module[{t, edges, res, ok},
  {t, res} = AbsoluteTiming[Module[{e = DDTMeshEdges[1]},
     DDTMeshBellPair[1, #]["isBellPair"] & /@ e]];
  ok = And @@ res;
...
Module[{tol = 0.15, allok = True},
  Do[Module[{r2, d},
     r2 = DDTPageRenyi2Ensemble[nn, 100, "Seed" -> 1000 + nn];
...
Module[{allok = True},
  Do[Module[{nn = c[[1]], sw = c[[2]], pc, ok},
     pc = DDTPageCurveScrambledState[nn, "Seed" -> 3, "Sweeps" -> sw];
...
Module[{t, sweep, fes, mono, ok},
  {t, sweep} = AbsoluteTiming[DDTHPNoiseSweep[sc["gates"], {0.005, 0.01, 0.015, 0.02}, 500, 11]];
  fes = #["meanFe"] & /@ sweep;
