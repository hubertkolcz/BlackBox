...
## Q1 — cct-optimality: can a Legendre theta-frontier close the window?
...
certificate `Gamma_k` and drive the sup gap toward `gap(cct) = 0.0698975`?
...
fc in [1/2, 1]**: the words `ct` (fc=1/2) and `ccct` (fc=3/4) both have exact
`theta-bar = 3/2`, and `theta-bar <= 3/2` is the global ceiling. cct sits at
`(fc=2/3, theta-bar=1.4032309)`, a **strict interior point 0.0967691 below the
hull**. Because cct is not on the concave hull, **no single supporting hyperplane
...
`Gamma_9 = 0.0720260`, so it cannot drive the sup gap toward gap(cct).
...
certifiable limit at ~0.076 off-cct). Both say the same thing: linear/affine
...
- Anchors (established, cited not re-derived): `gap(cct)=0.0698975` (irrational);
  `Gamma_10=0.0714575` (numeric), `Gamma_9=0.0720260` (exact); alpha-cis theorem
  `alpha-bar(w) >= max(4/3, 1+f_c/2)`, equality at 4/3 iff `w=(cct)^k`.

### WHAT REMAINS
The obstruction closes the **Legendre/affine-in-fc route** specifically. The
cct-optimality window `[gap(cct)=0.0699, Gamma_9=0.0720]` — the interval between
...
invariant than `(fc, theta-bar)` in which cct becomes extremal. Any method whose
...
nonlinear in fc, with `sup_w Phi = gap(cct)` attained only on `(cct)^k`?
...
- **The orbit-spectrum reading (cct).** Seed values = orbit density − 1 (cis 3/2, trans
  `tau* = Root[49x^3-128x^2-75x+218,2]`, 16/11, 19/13, 25/17); `gap(cct)=0.0698975`
  irrational; alpha-cis theorem equality at 4/3 iff `w=(cct)^k`. This session proves the
  Legendre/affine-in-fc enrichment **cannot expose cct** (it is a strict interior point of
  the flat-at-3/2 concave hull). **[established + theorem-obstruction]**
- **The exact mesh laws.** `alpha-bar(w) >= max(4/3, 1+f_c/2)`; census exhaustive to
  period 18; balanced/Sturmian words >= 0.012 below cct; affine alpha-credit tilt proven
  inert. **[established]**
...
- **cct-optimality window `[0.0699, 0.0720]`:** open. Legendre/affine-in-fc enrichment
...
passes. All numeric anchors re-confirmed this session (cct hull, Paley LP, ERG-003
