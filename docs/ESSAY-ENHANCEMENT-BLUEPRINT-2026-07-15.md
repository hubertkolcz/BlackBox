...
| A4 | `composition-optimality/orbit_spectrum.png` (`.pdf` twin) | **Stage 8** (Ergodic Optimization) | 2-panel: (A) orbit-density spectrum from **twisted τ\*=1.3767** to **direct 3/2**, crowding zone 16/11, 19/13, 25/17; (B) seeds vs density — **ddt alone drops to density−4/3, gap 0.0698975**. | `composition-optimality/orbit_spectrum_figure.wl` header |
...
| A7 | `optical-synthesis/schematics/` — **strip of 4**: `demo1_kcbs_pentagon_L1.png`, `demo2_c7_heptagon_L1.png`, `demo3_ddt_mesh_reps2.png`, `demo4_table_V0977_L2.png` | **Stage 6** (Optical Constructor) | The compiler's actual output blueprints: KCBS L1 cascade (cosθ=1/φ, genuine so(3) DLA=3); C7 L1 (identity S=7−4θ(C7)); ddt-mesh reps=2 (18 modes); pure table V=977/1000 **Layer-2 only, leaf-confined**. | `optical-synthesis/DESIGN.md`; `blueprints/*.wl` (`kcbs_L1/L2`, `mesh_ddt2` are alt renders) |
